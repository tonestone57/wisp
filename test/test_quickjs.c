...
START_TEST(test_quickjs_document)
{
    ...
    const char *code_cookie =
        "document.cookie = 'foo=bar; path=/';\n"
        "document.cookie === 'foo=bar';";
    result = js_exec(thread, (const uint8_t *)code_cookie, strlen(code_cookie), "test_document_cookie");
    ck_assert(result == true);

    /* Test document.write */
    const char *code_write =
        "document.write('<div id=\"written-element\" class=\"test-class1 test-class2\">Hello</div>');\n"
        "var wr = document.getElementById('written-element');\n"
        "wr !== null && wr.tagName === 'DIV' && wr.innerHTML === 'Hello';";
    result = js_exec(thread, (const uint8_t *)code_write, strlen(code_write), "test_document_write");
    ck_assert(result == true);

    /* Test classList */
    const char *code_classlist =
        "var wr = document.getElementById('written-element');\n"
        "var cl = wr.classList;\n"
        "var lenOk = cl.length === 2;\n"
        "var itemOk = cl.item(0) === 'test-class1' && cl.item(1) === 'test-class2';\n"
        "var contOk = cl.contains('test-class1') && !cl.contains('other');\n"
        "cl.add('new-class');\n"
        "var addOk = cl.contains('new-class') && cl.length === 3;\n"
        "cl.remove('test-class2');\n"
        "var remOk = !cl.contains('test-class2') && cl.length === 2;\n"
        "cl.toggle('new-class');\n" // removes it
        "var toggleRemOk = !cl.contains('new-class') && cl.length === 1;\n"
        "cl.toggle('new-class');\n" // adds it
        "var toggleAddOk = cl.contains('new-class') && cl.length === 2;\n"
        "lenOk && itemOk && contOk && addOk && remOk && toggleRemOk && toggleAddOk;";
    result = js_exec(thread, (const uint8_t *)code_classlist, strlen(code_classlist), "test_element_classlist");
    ck_assert(result == true);

    /* Test NamedNodeMap attributes */
    const char *code_attributes =
        "var wr = document.getElementById('written-element');\n"
        "var attrs = wr.attributes;\n"
        "var lenOk = attrs.length >= 2;\n" // 'id' and 'class'
        "var idAttr = attrs.getNamedItem('id');\n"
        "var idOk = idAttr !== null && idAttr.name === 'id' && idAttr.value === 'written-element';\n"
        "var itemOk = attrs.item(0) !== null;\n"
        "lenOk && idOk && itemOk;";
    result = js_exec(thread, (const uint8_t *)code_attributes, strlen(code_attributes), "test_element_attributes");
    ck_assert(result == true);

    /* Test Element style proxy */
    const char *code_style =
        "var wr = document.getElementById('written-element');\n"
        "wr.style.color = 'red';\n"
        "wr.style.backgroundColor = 'blue';\n"
        "wr.style.color === 'red' && wr.style.backgroundColor === 'blue' && wr.getAttribute('style').indexOf('color: red') !== -1;";
    result = js_exec(thread, (const uint8_t *)code_style, strlen(code_style), "test_element_style_proxy");
    ck_assert(result == true);
