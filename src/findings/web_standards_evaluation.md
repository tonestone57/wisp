...
| **JavaScript (ES2023+)** | **~98.5% (Web APIs)** <br> **100% (Language)** | Integrated **QuickJS-ng v0.15.1** (full ES2023+ compliance), Web Workers with structured cloning, Web Crypto (OpenSSL/LibreSSL), Storage & Files (IndexedDB, FileReader, Blob, File), Media & Speech APIs (canPlayType, VTTCue, TextTrack, MediaDevices, SpeechSynthesis), Real-time & Messaging (WebRTC, WebSocket, EventSource, BroadcastChannel, SharedWorker, MessageChannel), Geolocation & Hardware Sensors (Gamepad, Battery, Vibration, DeviceMotion/Orientation), and **2514+ manual WebIDL stub overrides (3008+ across the engine)**. | Understudied modern Bluetooth/USB APIs. |
...
*   **WebIDL Bindings**:
    ...
    *   **3008+ manual WebIDL stub overrides**: Fully implemented manuals (strong C overrides) covering core HTMLElement/Location/History/Document properties and interfaces (with 2514+ in `stubs_manual_impl.c` alone), accompanied by rigorous unit/integration test coverage with 100% pass rates.
    *   **Storage & Files APIs**: `Blob`, `File`, `FileReader` (with state constants, `readAsDataURL`, `readAsArrayBuffer`, `readAsText`, `readAsBinaryString`), `URL.createObjectURL`/`revokeObjectURL`, and `IndexedDB` family (`IDBFactory`, `IDBOpenDBRequest`, `IDBDatabase`, `IDBTransaction`, `IDBObjectStore`, `IDBRequest`).
    *   **Web Crypto & Security**: `window.crypto.subtle` Web Crypto API backed by LibreSSL/OpenSSL, `HTMLIFrameElement.sandbox` token list binding, and `srcdoc` attribute parsing.
    *   **Media, Speech & Hardware Sensors**: `canPlayType` MIME/codec resolution (MP4, WebM, Ogg, MP3, AAC, WAV, Opus, FLAC, AV1, AV2), non-DOM node private wrapping for `Audio`, `TextTrack`, `VTTCue`, `MediaDevices`, `SpeechSynthesis` voices, `navigator.geolocation`, `DeviceOrientation`/`Motion`, `Gamepad`, `BatteryManager`, and Vibration APIs.
    *   **Real-time, Communication & Editing**: WebRTC (`RTCPeerConnection`, `RTCDataChannel`), `MessageChannel`/`MessagePort`, `SharedWorker`, `BroadcastChannel`, `WebSocket`, `EventSource`, `contentEditable`, `document.designMode`, `draggable`, and tag-to-constructor HTML5 prototype dispatch in `qjs_new_element`.
