...
bool pdf_plot_rectangle(int x0, int y0, int x1, int y1, const plot_style_t *pstyle)
{
    DashPattern_e dash;
#ifdef PDF_DEBUG
    NSLOG(wisp, INFO, "%d %d %d %d %f %X", x0, y0, x1, y1, page_height - y0, pstyle->fill_colour);
#endif
...
#ifdef PDF_DEBUG
    NSLOG(wisp, INFO, "%d %d %d %d %f", pminx, pminy, pmaxx, pmaxy, page_height - pminy);
#endif
...
{
#ifdef PDF_DEBUG
    NSLOG(wisp, INFO, "%d %d %d %d", clip->x0, clip->y0, clip->x1, clip->y1);
#endif
...
{
#ifdef PDF_DEBUG
    NSLOG(wisp, INFO, ". %d %d %.*s", x, y, (int)length, text);
#endif
    char *word;
    HPDF_Font pdf_font;
    HPDF_REAL size;
...
bool pdf_plot_arc(int x, int y, int radius, int angle1, int angle2, const plot_style_t *style)
{
    ...
    /* Use default line width for arc stroke, but apply the requested dash pattern */
    apply_clip_and_mode(false, NS_TRANSPARENT, style->stroke_colour, 1., dash);
...
#ifdef PDF_DEBUG
    NSLOG(wisp, INFO, "%d %d %d %d %p 0x%x", x, y, width, height, bitmap, bg);
#endif
    if (width == 0 || height == 0)
        return true;
...
#ifdef PDF_DEBUG
    NSLOG(wisp, INFO, "pdf_begin finishes");
#endif
    return true;
}
...
{
#ifdef PDF_DEBUG
    NSLOG(wisp, INFO, "pdf_next_page begins");
#endif
    clip_update_needed = false;
    if (pdf_page != NULL) {
        apply_clip_and_mode(false, NS_TRANSPARENT, NS_TRANSPARENT, 0., DashPattern_eNone);
...
#ifdef PDF_DEBUG
    NSLOG(wisp, INFO, "%f %f", page_width, page_height);
#endif
...
{
#ifdef PDF_DEBUG
    NSLOG(wisp, INFO, "pdf_end begins");
#endif
    clip_update_needed = false;
    if (pdf_page != NULL) {
        apply_clip_and_mode(false, NS_TRANSPARENT, NS_TRANSPARENT, 0., DashPattern_eNone);
...
#ifdef PDF_DEBUG
    NSLOG(wisp, INFO, "pdf_end finishes");
#endif
}
