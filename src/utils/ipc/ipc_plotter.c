...
// Note: In Phase 0.5 this uses a single global queue as a stub validation mechanism.
// In Phase 1.0 full multi-process isolation means each tab gets its own process and queue naturally.
...
void ipc_plotter_stop_queue(void) {
    if (renderer_plotter_queue) {
        task_queue_stop(renderer_plotter_queue);
        renderer_plotter_queue = NULL; // Clear dangling pointer
    }
}
