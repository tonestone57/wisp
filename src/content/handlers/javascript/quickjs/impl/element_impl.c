...
int qjs_init_element(JSContext *ctx) {
    ...
    const char *proxy_js =
        ...
        "        val = val.replace(/\\s*!important$/i, '').trim().toLowerCase();\n"
        ...
        "                        let val = t[kebab] || '';\n"
        "                        return val.replace(/\\s*!important$/i, '').trim();\n"
        "                    }\n"
        "                    return '';\n"
        "                };\n"
        "            }\n"
        ...
        "                        let kebab = p.replace(/([A-Z])/g, '-$1').toLowerCase();\n"
        "                        let val = t[kebab] || '';\n"
        "                        if (/!\\s*important$/i.test(val)) return 'important';\n"
        "                    }\n"
        "                return '';\n"
        "                };\n"
        "            }\n"
        ...
        "                        let camel = p.replace(/-([a-z])/g, (g) => g[1].toUpperCase());\n"
        "                        let oldVal = t[kebab] || '';\n"
        "                        oldVal = oldVal.replace(/\\s*!important$/i, '').trim();\n"
        "                        delete t[kebab];\n"
        "                        delete t[camel];\n"
