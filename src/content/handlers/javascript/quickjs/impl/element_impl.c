...
static JSValue js_element_get_layout_property_global(JSContext *ctx, JSValueConst this_val, int argc, JSValueConst *argv) {
    ...
    bool node_layout_dirty = (wisp_shm_dom && sn && (sn->layout_dirty != 0 || (sn->layout_index != 0 &&
    ...
    if (rw <= 0 || rh <= 0) {
        ...
        if (strcasecmp(tag, "details") == 0) {
            ...
            if (JS_IsString(wv)) {
                const char *ws = JS_ToCString(ctx, wv);
                if (ws) { rw = atoi(ws); JS_FreeCString(ctx, ws); }
            }
            if (JS_IsString(hv)) {
                const char *hs = JS_ToCString(ctx, hv);
                if (hs) { rh = atoi(hs); JS_FreeCString(ctx, hs); }
            }
            JS_FreeValue(ctx, wv);
            JS_FreeValue(ctx, hv);
            if (rw <= 0) rw = 300;
    ...
    if (wisp_is_js_process) {
        ...
        if (sn && type == DOM_ELEMENT_NODE) {
            ...
            if (tag) {
                ...
                if (strcasecmp(tag, "h1") == 0 || strcasecmp(tag, "h2") == 0 || strcasecmp(tag, "h3") == 0 || strcasecmp(tag, "h4") == 0 || strcasecmp(tag, "h5") == 0 || strcasecmp(tag, "h6") == 0) {
