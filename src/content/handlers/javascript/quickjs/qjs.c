...
static void qjs_inject_fetch_polyfill(JSContext *ctx)
{
    const char *fetch_polyfill =
        ...
        "            this.label = label || (this.kind === 'video' ? 'Default Camera' : 'Default Microphone');\n"
        "            this.enabled = true;\n"
        ...
        "            const self = this;\n"
        "            setTimeout(function() {\n"
        "                const evt = new globalThis.Event('ended');\n"
        "                self.dispatchEvent(evt);\n"
        "                if (typeof self.onended === 'function') {\n"
        "                    try { self.onended(evt); } catch(e) {}\n"
        "                }\n"
        "            }, 0);\n"
        "        }\n"
        "    };\n"
        ...
        "                this._tracks = arg.getTracks().map(t => new globalThis.MediaStreamTrack(t.kind, t.label));\n"
        "            } else if (Array.isArray(arg)) {\n"
        "                this._tracks = arg.slice();\n"
        "            }\n"
        "        }\n"
        "        get active() {\n"
        ...
        "                const self = this;\n"
        "                setTimeout(function() {\n"
        "                    const evt = new globalThis.Event('addtrack');\n"
        "                    evt.track = track;\n"
        "                    self.dispatchEvent(evt);\n"
        "                    if (typeof self.onaddtrack === 'function') {\n"
        "                        try { self.onaddtrack(evt); } catch(e) {}\n"
        "                    }\n"
        "                }, 0);\n"
        "            }\n"
        "        }\n"
        ...
        "                const self = this;\n"
        "                setTimeout(function() {\n"
        "                    const evt = new globalThis.Event('removetrack');\n"
        "                    evt.track = track;\n"
        "                    self.dispatchEvent(evt);\n"
        "                    if (typeof self.onremovetrack === 'function') {\n"
        "                        try { self.onremovetrack(evt); } catch(e) {}\n"
        "                    }\n"
        "                }, 0);\n"
        "            }\n"
        "        }\n"
        "        clone() {\n"
        "            const clonedTracks = this._tracks.map(t => {\n"
        "                const nt = new globalThis.MediaStreamTrack(t.kind, t.label);\n"
        "                nt.enabled = t.enabled;\n"
        "                nt.readyState = t.readyState;\n"
        "                return nt;\n"
        "            });\n"
        ...
        "\n"
        "(function() {\n"
        "    const MediaDevicesClass = class extends globalThis.EventTarget {\n"
        "        constructor() {\n"
        "            super();\n"
        ...
        "        enumerateDevices() {\n"
        "            return Promise.resolve([\n"
        "                { kind: 'audioinput', label: 'Default Microphone', deviceId: 'mock-mic', groupId: 'group-1' },\n"
        "                { kind: 'videoinput', label: 'Default Camera', deviceId: 'mock-cam', groupId: 'group-2' }\n"
        "            ]);\n"
        "        }\n"
        ...
        "            return new Promise(function(resolve, reject) {\n"
        "                const tracks = [];\n"
        "                const audioReq = constraints && (constraints.audio || constraints.audio === undefined);\n"
        "                const videoReq = constraints && (constraints.video || constraints.video === undefined);\n"
        "                if (audioReq) {\n"
        "                    tracks.push(new globalThis.MediaStreamTrack('audio', 'Default Microphone'));\n"
        "                }\n"
        "                if (videoReq) {\n"
        "                    tracks.push(new globalThis.MediaStreamTrack('video', 'Default Camera'));\n"
        "                }\n"
        "                resolve(new globalThis.MediaStream(tracks));\n"
        "            });\n"
        "        }\n"
        "        getDisplayMedia(constraints) {\n"
        "            return new Promise(function(resolve, reject) {\n"
        "                const tracks = [new globalThis.MediaStreamTrack('video', 'Screen Share')];\n"
        "                resolve(new globalThis.MediaStream(tracks));\n"
        "            });\n"
        "        }\n"
        "    };\n"
        "    \n"
        "    const mediaDevicesInstance = new MediaDevicesClass();\n"
        "    \n"
        "    if (globalThis.navigator) {\n"
        "        Object.defineProperty(globalThis.navigator, 'mediaDevices', {\n"
        "            value: mediaDevicesInstance,\n"
        "            writable: false, configurable: false, enumerable: true\n"
        "        });\n"
        "    }\n"
        "    if (globalThis.Navigator && globalThis.Navigator.prototype) {\n"
        "        Object.defineProperty(globalThis.Navigator.prototype, 'mediaDevices', {\n"
        "            get() { return mediaDevicesInstance; },\n"
        "            configurable: true, enumerable: true\n"
        "        });\n"
        "    }\n"
        "})();\n";
