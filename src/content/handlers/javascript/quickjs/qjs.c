...
void qjs_inject_dom_polyfills(JSContext *ctx)
{
    ...
    const char *html5_polyfills =
        ...
        "if (typeof globalThis.TrackDefault === 'undefined') {\n"
        "    globalThis.TrackDefault = class TrackDefault {\n"
        "        constructor(type, language, label, kinds, byteStreamTrackID) {\n"
        "            this.type = type;\n"
        "            this.language = language || '';\n"
        "            this.label = label || '';\n"
        "            this.kinds = kinds || [];\n"
        "            this.byteStreamTrackID = byteStreamTrackID || '';\n"
        "        }\n"
        "    };\n"
        "}\n"
        "if (typeof globalThis.TrackDefaultList === 'undefined') {\n"
        "    globalThis.TrackDefaultList = class TrackDefaultList {\n"
        "        constructor(trackDefaults) {\n"
        "            this._list = Array.isArray(trackDefaults) ? trackDefaults.slice() : [];\n"
        "        }\n"
        ...
        "            this.mode = 'segments';\n"
        "            this.trackDefaults = new globalThis.TrackDefaultList();\n"
        ...
        "                self.dispatchEvent(new globalThis.Event('updatestart'));\n"
        "            }\n"
        "            let bytes = 0;\n"
        ...
        "                bytes = data.length;\n"
        "            }\n"
        "            let addedSecs = Math.max(0.1, bytes > 0 ? bytes / 65536.0 : 0.5);\n"
        "            let prevEnd = this.buffered && this.buffered.length > 0 ? this.buffered.end(0) : this.timestampOffset;\n"
        "            let newEnd = prevEnd + addedSecs;\n"
        "            let startPos = this.timestampOffset;\n"
        "            this.buffered = {\n"
        "                length: 1,\n"
        "                start(i) { return startPos; },\n"
        "                end(i) { return newEnd; }\n"
        "            };\n"
        "            setTimeout(function() {\n"
        "                self.updating = false;\n"
        "                if (typeof self.dispatchEvent === 'function') {\n"
        "                    self.dispatchEvent(new globalThis.Event('update'));\n"
        "                    self.dispatchEvent(new globalThis.Event('updateend'));\n"
        "                }\n"
        "            }, 0);\n"
        "        }\n"
        "        remove(start, end) {\n"
        ...
        "                this.dispatchEvent(new globalThis.Event('updateend'));\n"
        "            }\n"
        "        }\n"
        "        changeType(type) {\n"
        "            this.type = type;\n"
        ...
        "        _add(sb) {\n"
        "            this._list.push(sb);\n"
        "            this[this._list.length - 1] = sb;\n"
        "            if (typeof this.dispatchEvent === 'function') {\n"
        "                const evt = new globalThis.Event('addsourcebuffer');\n"
        "                evt.sourceBuffer = sb;\n"
        ...
        "                if (typeof this.dispatchEvent === 'function') {\n"
        "                    const evt = new globalThis.Event('removesourcebuffer');\n"
        "                    evt.sourceBuffer = sb;\n"
        "                    this.dispatchEvent(evt);\n"
        "                }\n"
        "            }\n"
        ...
        "            if (t.startsWith('video/mp4') || t.startsWith('video/webm') ||\n"
        "                t.startsWith('audio/mp4') || t.startsWith('audio/webm') ||\n"
        "                t.startsWith('video/ogg') || t.startsWith('audio/ogg') ||\n"
        "                t.startsWith('video/mp2t') || t.startsWith('audio/aac') ||\n"
        "                t.startsWith('audio/mpeg') || t.startsWith('audio/flac') ||\n"
        "                t.startsWith('video/h264') || t.startsWith('video/hevc')) {\n"
        "                return true;\n"
        "            }\n"
        "            return false;\n"
        "        }\n"
        ...
        "            if (!type || typeof type !== 'string') return false;\n"
        "            try {\n"
        "                var video = (typeof globalThis.document !== 'undefined' && globalThis.document) ? globalThis.document.createElement('video') : null;\n"
        ...
        "            this.readyState = 'ended';\n"
        "            if (typeof this.dispatchEvent === 'function') {\n"
        "                this.dispatchEvent(new globalThis.Event('sourceended'));\n"
        "            }\n"
        "        }\n"
        "        setLiveSeekableRange(start, end) {\n"
        "            this._liveSeekableRange = { start: start, end: end };\n"
        "        }\n"
        "        clearLiveSeekableRange() {\n"
        "            this._liveSeekableRange = null;\n"
        "        }\n"
        "    };\n"
        "}\n"
        "if (typeof globalThis.VideoPlaybackQuality === 'undefined') {\n"
        "    globalThis.VideoPlaybackQuality = class VideoPlaybackQuality {\n"
        "        constructor() {\n"
        "            this.creationTime = (typeof performance !== 'undefined' && performance.now) ? performance.now() : Date.now();\n"
        ...
        "        SourceBufferList: globalThis.SourceBufferList,\n"
        "        VideoPlaybackQuality: globalThis.VideoPlaybackQuality,\n"
        "        TrackDefault: globalThis.TrackDefault,\n"
        "        TrackDefaultList: globalThis.TrackDefaultList,\n"
        "        WebKitMediaSource: globalThis.MediaSource,\n"
        "        mozMediaSource: globalThis.MediaSource,\n"
        "        msMediaSource: globalThis.MediaSource,\n"
        "        ManagedMediaSource: globalThis.MediaSource\n"
        "    };\n"
        "    Object.keys(MSEClasses).forEach(k => {\n"
        "        if (typeof globalThis !== 'undefined' && !(k in globalThis)) {\n"
        "            globalThis[k] = MSEClasses[k];\n"
        "        }\n"
        "        if (typeof window !== 'undefined' && !(k in window)) {\n"
        "            window[k] = MSEClasses[k];\n"
        "        }\n"
        "        if (typeof Window !== 'undefined' && Window.prototype && !(k in Window.prototype)) {\n"
        "            Window.prototype[k] = MSEClasses[k];\n"
        "        }\n"
        "    });\n"
        ...
        "                return new globalThis.VideoPlaybackQuality();\n"
        "            };\n"
        "        }\n"
        "    }\n"
        "})();\n"
