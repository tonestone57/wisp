...
void qjs_inject_dom_polyfills(JSContext *ctx)
{
    ...
    const char *html5_polyfills =
        ...
        "    globalThis.FontFaceSet = class FontFaceSet extends globalThis.EventTarget {\n"
        "        constructor() {\n"
        "            super();\n"
        "            this._fonts = new Set();\n"
        "            this.status = 'loaded';\n"
        "            this.ready = Promise.resolve(this);\n"
        "        }\n"
        "        add(font) { this._fonts.add(font); return this; }\n"
        "        delete(font) { return this._fonts.delete(font); }\n"
        "        clear() { this._fonts.clear(); }\n"
        "        has(font) { return this._fonts.has(font); }\n"
        "        check(font, text) { return true; }\n"
        "        load(font, text) { return Promise.resolve(Array.from(this._fonts)); }\n"
        "        get size() { return this._fonts.size; }\n"
        "        [Symbol.iterator]() { return this._fonts[Symbol.iterator](); }\n"
        "        forEach(cb, thisArg) { this._fonts.forEach(cb, thisArg); }\n"
        "    };\n"
        "}\n"
        "if (typeof document !== 'undefined' && !document.fonts) {\n"
        "    document.fonts = new globalThis.FontFaceSet();\n"
        "}\n"
        "\n"
