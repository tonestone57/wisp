...
#endif
#ifdef _WIN32
    EnterCriticalSection(&pool->lock); pool->active_workers++; LeaveCriticalSection(&pool->lock);
#else
    pthread_mutex_lock(&pool->lock); pool->active_workers++; pthread_mutex_unlock(&pool->lock);
#endif
}
...
static void init_pool(WispPool *pool, int worker_count, int queue_size, bool is_js) {
    ...
    pool->is_js = is_js;
#ifdef _WIN32
    InitializeCriticalSection(&pool->lock); InitializeConditionVariable(&pool->cond);
#else
    pthread_mutex_init(&pool->lock, NULL); pthread_cond_init(&pool->cond, NULL);
#endif
    if (worker_count > 0) {
...
static void shutdown_pool(WispPool *pool) {
    if (pool->workers == NULL && pool->worker_count == 0) return;
#ifdef _WIN32
    EnterCriticalSection(&pool->lock);
    pool->stop = true;
    for (int i = 0; i < pool->worker_count; i++) pool->workers[i].running = false;
...
#ifdef _WIN32
        if (pool->workers[i].thread) { WaitForSingleObject(pool->workers[i].thread, INFINITE); CloseHandle(pool->workers[i].thread); }
#else
        pthread_t null_thread; memset(&null_thread, 0, sizeof(pthread_t));
        if (memcmp(&pool->workers[i].thread, &null_thread, sizeof(pthread_t)) != 0) pthread_join(pool->workers[i].thread, NULL);
#endif
        if (pool->workers[i].ctx) JS_FreeContext(pool->workers[i].ctx);
        if (pool->workers[i].rt) { JS_RunGC(pool->workers[i].rt); JS_FreeRuntime(pool->workers[i].rt); }
    }
    free(pool->workers);
    js_task_t *task = pool->head;
    while (task) { js_task_t *next = task->next; if (task->script) free(task->script); free(task); task = next; }
#ifdef _WIN32
    DeleteCriticalSection(&pool->lock);
#else
    pthread_mutex_destroy(&pool->lock); pthread_cond_destroy(&pool->cond);
#endif
    int is_js = pool->is_js; memset(pool, 0, sizeof(WispPool)); pool->is_js = is_js;
}

void init_wisp_subsystem(int queue_size) {
    if (raster_pool.worker_count > 0 || js_pool.worker_count > 0 || raster_pool.workers != NULL) return;
    long n_cores;
#ifdef _WIN32
    SYSTEM_INFO sysinfo; GetSystemInfo(&sysinfo); n_cores = sysinfo.dwNumberOfProcessors;
#else
...
void* wisp_worker_routine(void *arg) {
    ...
    while (worker->running) {
        js_task_t *task = NULL; bool has_task = false;
#ifdef _WIN32
        EnterCriticalSection(&pool->lock);
        while (pool->head == NULL && worker->running && !pool->stop) {
            if (!SleepConditionVariableCS(&pool->cond, &pool->lock, 5000) && GetLastError() == ERROR_TIMEOUT) {
                if (pool->head == NULL && pool->active_workers > 1) {
                    worker->running = false; pool->active_workers--;
                    ...
                    LeaveCriticalSection(&pool->lock);
                    if (h) CloseHandle(h); return NULL;
                }
            }
        }
        if (pool->head && worker->running) {
            task = pool->head; pool->head = task->next; if (!pool->head) pool->tail = NULL;
        ...
        while (pool->head == NULL && worker->running && !pool->stop) {
            ...
            if (pthread_cond_timedwait(&pool->cond, &pool->lock, &ts) != 0) {
                if (pool->head == NULL && pool->active_workers > 1) {
                    ...
                    pthread_mutex_unlock(&pool->lock); pthread_detach(pthread_self()); return NULL;
                }
            }
        }
        if (pool->head && worker->running) {
        ...
        if (has_task && task) {
            if (task->function) task->function(task->arg);
                ...
                if (JS_IsException(res)) {
                    JSValue exc = JS_GetException(worker->ctx); const char *exc_str = JS_ToCString(worker->ctx, exc);
                    if (exc_str) { NSLOG(wisp, WARNING, "Worker JS Error: %s", exc_str); JS_FreeCString(worker->ctx, exc_str); }
                    JS_FreeValue(worker->ctx, exc);
                }
                JS_FreeValue(worker->ctx, res); JS_RunGC(worker->rt);
            }
            if (task->script) free(task->script); free(task);
#ifdef _WIN32
...
static void wisp_dispatch_internal(WispPool *pool, char *script, void (*func)(void*), void *arg) {
    if (pool->worker_count == 0) { if (func) func(arg); if (script) free(script); return; }
    js_task_t *new_task = malloc(sizeof(js_task_t)); if (!new_task) { if (script) free(script); return; }
    new_task->next = NULL; new_task->script = script; new_task->function = func; new_task->arg = arg;
#ifdef _WIN32
    EnterCriticalSection(&pool->lock);
    if (!pool->stop && pool->count < pool->capacity) {
        if (!pool->tail) { pool->head = new_task; pool->tail = new_task; }
        else { pool->tail->next = new_task; pool->tail = new_task; }
        pool->count++;
        int worker_to_start = -1;
        if (pool->busy_workers == pool->active_workers && pool->active_workers < pool->worker_count) {
            for (int i=0; i<pool->worker_count; i++) { if (!pool->workers[i].running) { worker_to_start = i; pool->workers[i].running = true; break; } }
        }
        WakeConditionVariable(&pool->cond); LeaveCriticalSection(&pool->lock);
        if (worker_to_start != -1) start_worker_in_pool(pool, worker_to_start);
    } else { LeaveCriticalSection(&pool->lock); if (new_task->script) free(new_task->script); free(new_task); }
#else
    pthread_mutex_lock(&pool->lock);
    if (!pool->stop && pool->count < pool->capacity) {
        if (!pool->tail) { pool->head = new_task; pool->tail = new_task; }
        ...
        if (pool->busy_workers == pool->active_workers && pool->active_workers < pool->worker_count) {
            pthread_t null_thread; memset(&null_thread, 0, sizeof(pthread_t));
            for (int i=0; i<pool->worker_count; i++) { if (!pool->workers[i].running && memcmp(&pool->workers[i].thread, &null_thread, sizeof(pthread_t)) == 0) { worker_to_start = i; pool->workers[i].running = true; break; } }
        }
        pthread_cond_signal(&pool->cond); pthread_mutex_unlock(&pool->lock);
        if (worker_to_start != -1) start_worker_in_pool(pool, worker_to_start);
    } else { pthread_mutex_unlock(&pool->lock); if (new_task->script) free(new_task->script); free(new_task); }
#endif
}
