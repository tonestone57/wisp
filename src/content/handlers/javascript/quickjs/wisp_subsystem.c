...
static void shutdown_pool(WispPool *pool)
{
    ...
    WispWorker *captured_workers = NULL;
    int worker_count = 0;
    ...
    if (worker_count > 0) {
        captured_workers = malloc(worker_count * sizeof(WispWorker));
        memcpy(captured_workers, pool->workers, worker_count * sizeof(WispWorker));
    ...
    if (worker_count > 0) {
        ...
        for (int i = 0; i < worker_count; i++) {
            pool->workers[i].running = false;
            memset(&pool->workers[i].thread, 0, sizeof(pthread_t));
            pool->workers[i].ctx = NULL;
            pool->workers[i].rt = NULL;
        }
    }
    ...
    if (captured_workers) {
        for (int i = 0; i < worker_count; i++) {
#ifdef _WIN32
            if (captured_workers[i].thread) {
                WaitForSingleObject(captured_workers[i].thread, INFINITE);
                CloseHandle(captured_workers[i].thread);
            }
#else
            pthread_t null_thread;
            memset(&null_thread, 0, sizeof(pthread_t));
            if (memcmp(&captured_workers[i].thread, &null_thread, sizeof(pthread_t)) != 0) {
                pthread_join(captured_workers[i].thread, NULL);
            }
#endif
            if (captured_workers[i].ctx != NULL) {
                JS_FreeContext(captured_workers[i].ctx);
            }
            if (captured_workers[i].rt != NULL) {
                JS_RunGC(captured_workers[i].rt);
                JS_FreeRuntime(captured_workers[i].rt);
            }
        }
        free(captured_workers);
    }
    free(pool->workers);
    pool->workers = NULL;
