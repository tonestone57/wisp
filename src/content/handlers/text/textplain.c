    ...
    int height, float scale, textplain_content *text, const struct selection *sel, const struct redraw_context *ctx)
{
    ...
    if (ctx->interactive == true) {
        ...
        if ((text->base.textsearch.context != NULL) &&
            content_textsearch_ishighlighted(
                text->base.textsearch.context, offset, offset + len, &search_start_idx, &search_end_idx)) {
        ...
        if (highlighted) {
            ...
            for (size_t k = 0; k < unique_bounds - 1; k++) {
                ...
                if (seg_start_txt > 0) {
                    res = guit->layout->width(&textplain_style, utf8_text, seg_start_txt, &startx);
                    if (res != NSERROR_OK)
                        startx = 0;
                }

                if (seg_end_txt > 0) {
                    res = guit->layout->width(&textplain_style, utf8_text, seg_end_txt, &endx);
                    if (res != NSERROR_OK)
                        endx = 0;
                }
                ...
                colour sel_bg = textplain_style.foreground;
