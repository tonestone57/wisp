 ...
 */
static void box_object_arena_destructor(void *vo)
{
 ...
 * \return 0 to allow talloc to continue destroying the tree.
 */
static void box_iframes_arena_destructor(void *ptr)
{
...
static bool box_input_text(html_content *html, struct box *box, struct dom_node *node)
{
	...
	inline_container = box_create(html, NULL, 0, false, 0, 0, 0, 0, html->bctx);
	if (!inline_container)
		return false;
	inline_container->type = BOX_INLINE_CONTAINER;
	inline_box = box_create(html, NULL, box->style, false, 0, 0, box->title, 0, html->bctx);
	if (!inline_box)
		return false;
...
static bool box_embed(dom_node *n, html_content *content, struct box *box, bool *convert_children)
{
	...
	arena_register_destructor(content->bctx, params, box_object_arena_destructor);
...
static bool box_iframe(dom_node *n, html_content *content, struct box *box, bool *convert_children)
{
	...
	arena_register_destructor(content->bctx, iframe, box_iframes_arena_destructor);
...
static bool box_input(dom_node *n, html_content *content, struct box *box, bool *convert_children)
{
	...
	if (dom_string_caseless_lwc_isequal(type, corestring_lwc_password)) {
		...
		inline_container = box_create(content, NULL, 0, false, 0, 0, 0, 0, content->bctx);
		if (inline_container == NULL)
			goto no_memory;

		inline_container->type = BOX_INLINE_CONTAINER;

		inline_box = box_create(content, NULL, box->style, false, 0, 0, box->title, 0, content->bctx);
		if (inline_box == NULL)
			goto no_memory;
