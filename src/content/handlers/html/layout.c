    ...
    for (row_group = table->children; row_group; row_group = row_group->next)
        for (row = row_group->children; row; row = row->next)
            for (cell = row->children; cell; cell = cell->next) {
                ...
                assert(cell->style);
                /** Note: Handle colspan="0" correctly.
    ...
    bool *line_has_height, const struct gui_layout_table *font_func, const html_content *content)
{
    ...
    *line_has_height = false;
    ...
    for (b = first; b; b = b->next) {
        ...
        if (b->type == BOX_INLINE && !b->object && !(b->flags & REPLACE_DIM) && !(b->flags & IFRAME)) {
            ...
            if (0 < fixed)
                max += fixed;
            *line_has_height = true;
            /* \Note  update min width, consider fractional extra */
        ...
        if (b->object || (b->flags & REPLACE_DIM)) {
            ...
            /* Note: handle percentage widths properly */
            if (width == AUTO)
                width = 400;
    ...
    if (first_line) {
        /* Note: handle percentage values properly */
        /* Note: handle text-indent interaction with floats */
...
layout_minmax_block(struct box *block, const struct gui_layout_table *font_func, const html_content *content)
{
    ...
    if (block->object) {
        ...
        if (block->width != UNKNOWN_WIDTH) {
            ...
            min = 400; /* Fallback iframe default constraints */
            max = 400;
        }
        block->flags |= HAS_HEIGHT;
    } else {
        ...
        for (child = block->children; child; child = child->next) {
            ...
            case BOX_TABLE:
                ...
                /* Note: fix for zero height tables */
                child_has_height = true;
...
bool layout_table(struct box *table, int available_width, html_content *content)
{
    ...
    /** \Note distribute spare height over the row groups / rows / cells */
    ...
    bool has_text_children, html_content *content, struct box **next_box)
{
    ...
    for (x = 0, b = first; x <= x1 - x0 && b != 0; b = b->next) {
        ...
        if (b->object && !(b->flags & REPLACE_DIM)) {
            ...
            /* Note: should we look at the content dimensions? */
            if (b->width == AUTO)
                b->width = 400;
            if (b->height == AUTO)
                b->height = 300;
    ...
    /** \Note  proper vertical alignment handling */
    for (d = first; d != b; d = d->next) {
    ...
    struct box *inline_container, int width, struct box *cont, int cx, int cy, html_content *content)
{
    ...
    /** \Note fix wrapping so that a box with horizontal scrollbar will
...
static bool layout_absolute(struct box *box, struct box *containing_block, int cx, int cy, html_content *content)
{
    ...
    if (box->type == BOX_BLOCK || box->type == BOX_INLINE_BLOCK || box->object || box->flags & IFRAME) {
        ...
        box->float_container = containing_block;
        /* \Note  layout_table considers margins etc. again */
...
static void layout_position_relative(const css_unit_ctx *unit_len_ctx, struct box *root, struct box *fp, int fx, int fy)
{
    ...
    /**\Note ensure containing box is large enough after moving boxes */
