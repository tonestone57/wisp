...
#define HTML5TEST_DATA_DIR "src/test/data/html5test"
    ...
    const char *shim_code =
        ...
        "          Object.defineProperty(self, 'response', { value: new ArrayBuffer(8), writable: true, configurable: true });\n"
        "        } else if (self.responseType === 'blob') {\n"
        "          Object.defineProperty(self, 'response', { value: new Blob([dummy_data]), writable: true, configurable: true });\n"
        "        }\n"
...
"  Test.prototype.stopBackground = function(id) {};\n"
    ...
    const char *print_err =
        ...
        "    if (parts[i].indexOf('elements') === 0 || parts[i].indexOf('components') === 0 || parts[i].indexOf('responsive') === 0 || parts[i].indexOf('storage') === 0) {\n"
        "      report.push(parts[i]);\n"
        "    }\n"
        "  }\n"
        "}\n"
