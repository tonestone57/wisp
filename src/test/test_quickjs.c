...
START_TEST(test_quickjs_css_style_declaration)
{
    ...
    const char *code =
        ...
        "if (typeof computed.getPropertyValue !== 'function') throw 'getComputedStyle should return CSSStyleDeclaration-like object';\n"
        "if (!('borderRadius' in computed)) throw 'borderRadius in computed check failed';\n"
        "if (computed.display !== 'block') throw 'computed display delegation failed';\n"
        "1;";
...
START_TEST(test_quickjs_css_stylesheet)
{
    ...
    const char *code =
        ...
        "if (sheet.cssRules[0].style.getPropertyValue('background-color') !== 'red') throw 'style rule parse failed';\n"
        "sheet.deleteRule(0);\n"
