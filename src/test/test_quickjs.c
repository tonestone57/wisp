...
START_TEST(test_quickjs_webidl_stubs)
{
    ...
    const char *script =
        ...
        "    \n"
        "        // 15. TimeRanges Tests\n"
        "        var tr = new TimeRanges([[1.5, 3.5], [5.0, 7.2]]);\n"
        "        if (tr.length !== 2) throw new Error('TimeRanges length failed');\n"
        "        if (tr.start(0) !== 1.5) throw new Error('TimeRanges start failed');\n"
        "        if (tr.end(1) !== 7.2) throw new Error('TimeRanges end failed');\n"
        "        var caught = false;\n"
        "        try { tr.start(5); } catch(e) { caught = true; }\n"
        "        if (!caught) throw new Error('TimeRanges OOB start failed to throw');\n"
        "\n"
        "        var media = document.createElement('video');\n"
        "        if (media.currentTime !== 0) throw new Error('HTMLMediaElement default currentTime failed');\n"
        "        media.currentTime = 2.5;\n"
        "        if (media.currentTime !== 2.5) throw new Error('HTMLMediaElement set currentTime failed');\n"
        "        if (media.paused !== true) throw new Error('HTMLMediaElement default paused failed');\n"
        "        media.play();\n"
        "        if (media.paused !== false) throw new Error('HTMLMediaElement play/paused failed');\n"
        "        media.pause();\n"
        "        if (media.paused !== true) throw new Error('HTMLMediaElement pause failed');\n"
        "        if (media.volume !== 1.0) throw new Error('HTMLMediaElement default volume failed');\n"
        "        media.volume = 0.5;\n"
        "        if (media.volume !== 0.5) throw new Error('HTMLMediaElement set volume failed');\n"
        "        caught = false;\n"
        "        try { media.volume = 1.5; } catch(e) { caught = true; }\n"
        "        if (!caught) throw new Error('HTMLMediaElement OOB volume failed to throw');\n"
        "        if (media.canPlayType('video/mp4') !== 'probably') throw new Error('HTMLMediaElement canPlayType failed');\n"
        "        if (media.canPlayType('unknown/foo') !== '') throw new Error('HTMLMediaElement canPlayType negative failed');\n"
        "        media.fastSeek(5.0);\n"
        "        if (media.currentTime !== 5.0) throw new Error('HTMLMediaElement fastSeek failed');\n"
        "    \n"
