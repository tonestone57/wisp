...
START_TEST(test_quickjs_form_datetime)
{
    ...
    const char *code =
        ...
        "  // Security validation checks for sscanf input validation:\n"
        "  // 1. Trailing garbage in date/datetime\n"
        "  input.type = 'date';\n"
        "  input.value = '2026-03-31EXTRA';\n"
        "  if (input.value !== '') throw 'trailing garbage in date allowed';\n"
        "  // 2. Out of bounds values\n"
        "  input.value = '2026-13-31';\n"
        "  if (input.value !== '') throw 'invalid month in date allowed';\n"
        "  input.value = '2026-03-35';\n"
        "  if (input.value !== '') throw 'invalid day in date allowed';\n"
        "  // 3. Trailing garbage in time\n"
        "  input.type = 'time';\n"
        "  input.value = '12:34:56GARBAGE';\n"
        "  if (input.value !== '') throw 'trailing garbage in time allowed';\n"
        "  // 4. Valid date passes\n"
        "  input.type = 'date';\n"
        "  input.value = '2026-03-31';\n"
        "  if (input.value !== '2026-03-31') throw 'valid date rejected';\n"
        "} catch(e) {\n"
...
START_TEST(test_quickjs_webidl_stubs)
{
    ...
    const char *script =
        ...
        "\n"
        "    // 7.5. HTMLElement dataset Tests\n"
        "    var div = document.createElement('div');\n"
        "    div.setAttribute('data-test-item', 'hello');\n"
        "    if (div.dataset === undefined) throw new Error('dataset property undefined');\n"
        "    if (div.dataset.testItem !== 'hello') throw new Error('dataset get property failed: ' + div.dataset.testItem);\n"
        "    div.dataset.testItem = 'world';\n"
        "    if (div.getAttribute('data-test-item') !== 'world') throw new Error('dataset set attribute failed');\n"
        "    if (div.dataset.nonExistent !== undefined) throw new Error('dataset nonExistent failed');\n"
        "    var protoDatasetGetter = Object.getOwnPropertyDescriptor(HTMLElement.prototype, 'dataset');\n"
        "    if (protoDatasetGetter && protoDatasetGetter.get) {\n"
        "        if (protoDatasetGetter.get.call(div) === undefined) throw new Error('HTMLElement.prototype.dataset getter failed for div');\n"
        "        var resUndefined;\n"
        "        try { resUndefined = protoDatasetGetter.get.call({}); } catch (e) { resUndefined = undefined; }\n"
        "        if (resUndefined !== undefined) throw new Error('HTMLElement.prototype.dataset getter should return undefined for invalid target');\n"
        "    }\n"
        "\n"
        ...
        "    if (svg_el.namespaceURI !== 'http://www.w3.org/2000/svg') throw new Error('svg namespaceURI failed: ' + svg_el.namespaceURI);\n"
        "    var math_el = document.createElement('math');\n"
        "    if (math_el.namespaceURI !== 'http://www.w3.org/1998/Math/MathML') throw new Error('math namespaceURI failed: ' + math_el.namespaceURI);\n"
        "\n"
        ...
        "    }\n"
        "\n"
        "    var imp_label = document.createElement('label');\n"
        "    var imp_inp = document.createElement('input');\n"
        "    imp_label.appendChild(imp_inp);\n"
        "    if (document.body) document.body.appendChild(imp_label);\n"
        "    if (!imp_inp.labels || imp_inp.labels.length !== 1 || imp_inp.labels[0] !== imp_label) {\n"
        "        throw new Error('HTMLInputElement.labels implicit association check failed');\n"
        "    }\n"
        "    if (imp_label.control !== imp_inp) {\n"
        "        throw new Error('HTMLLabelElement.control implicit association check failed');\n"
        "    }\n"
        "\n"
        ...
        "    if (video.canPlayType('video/mpeg') !== 'maybe') throw new Error('video.canPlayType mpeg failed');\n"
        "    if (video.canPlayType('video/3gpp') !== 'maybe') throw new Error('video.canPlayType 3gpp failed');\n"
        "    if (video.canPlayType('application/vnd.apple.mpegurl') !== 'maybe') throw new Error('video.canPlayType hls failed');\n"
        "    if (video.canPlayType('video/x-flv') !== 'maybe') throw new Error('video.canPlayType flv failed');\n"
        "    if (video.canPlayType('video/x-ms-wmv') !== 'maybe') throw new Error('video.canPlayType wmv failed');\n"
        "    if (video.canPlayType('image/avif') !== 'maybe') throw new Error('video.canPlayType avif failed');\n"
        "    if (video.canPlayType('video/mp4; codecs=\"dvh1.05.01\"') !== 'probably') throw new Error('video.canPlayType dolby vision failed');\n"
        "    if (video.canPlayType('video/mp4; codecs=\"vvc1.1.1\"') !== 'probably') throw new Error('video.canPlayType vvc failed');\n"
        "    if (video.canPlayType('video/mpeg; codecs=\"mp2v\"') !== 'probably') throw new Error('video.canPlayType mpeg2 video failed');\n"
        "    if (video.canPlayType('video/avi; codecs=\"mjpeg\"') !== 'probably') throw new Error('video.canPlayType mjpeg failed');\n"
        "    if (video.canPlayType('video/x-ms-wmv; codecs=\"wmv3\"') !== 'probably') throw new Error('video.canPlayType wmv3 failed');\n"
        "    if (video.canPlayType('video/quicktime; codecs=\"apch\"') !== 'probably') throw new Error('video.canPlayType prores failed');\n"
        ...
        "    if (audio.canPlayType('audio/aiff') === '') throw new Error('audio/aiff failed');\n"
        "    if (audio.canPlayType('audio/basic') === '') throw new Error('audio/basic failed');\n"
        "    if (audio.canPlayType('audio/3gpp') === '') throw new Error('audio/3gpp failed');\n"
        "    if (audio.canPlayType('audio/m4a; codecs=\"alac\"') !== 'probably') throw new Error('audio/m4a alac failed');\n"
        "    if (audio.canPlayType('audio/3gpp; codecs=\"amr\"') !== 'probably') throw new Error('audio/3gpp amr failed');\n"
        "    if (audio.canPlayType('audio/mp4; codecs=\"dts\"') !== 'probably') throw new Error('audio/mp4 dts failed');\n"
        "    if (audio.canPlayType('audio/x-ms-wma; codecs=\"wma\"') !== 'probably') throw new Error('audio/x-ms-wma wma failed');\n"
        "    if (!MediaSource.isTypeSupported('video/mp4; codecs=\"avc1.42E01E\"')) throw new Error('MediaSource.isTypeSupported valid failed');\n"
        "    if (!MediaSource.isTypeSupported('video/3gpp')) throw new Error('MediaSource.isTypeSupported 3gpp failed');\n"
        "    if (!MediaSource.isTypeSupported('application/vnd.apple.mpegurl')) throw new Error('MediaSource.isTypeSupported hls failed');\n"
...
START_TEST(test_quickjs_element_closest)
{
    ...
    const char *test_js =
        ...
        "if (detached.closest('body') !== null) throw new Error('Unattached node closest match should return null');\n"
