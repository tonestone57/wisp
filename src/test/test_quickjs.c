...
START_TEST(test_quickjs_media_source)
{
    ...
    const char *code =
        ...
        "if (!('VideoPlaybackQuality' in window)) throw new Error('VideoPlaybackQuality not in window');\n"
        "if (!('WebKitMediaSource' in window)) throw new Error('WebKitMediaSource not in window');\n"
        "if (!('mozMediaSource' in window)) throw new Error('mozMediaSource not in window');\n"
        "if (!('msMediaSource' in window)) throw new Error('msMediaSource not in window');\n"
        "if (typeof MediaSource.isTypeSupported !== 'function') throw new Error('isTypeSupported not a function');\n"
        "if (!MediaSource.isTypeSupported('video/mp4; codecs=\"avc1.42E01E\"')) throw new Error('isTypeSupported returned false for mp4');\n"
        "if (!MediaSource.isTypeSupported('video/webm; codecs=\"vp9\"')) throw new Error('isTypeSupported returned false for webm');\n"
        "if (MediaSource.isTypeSupported('text/plain')) throw new Error('isTypeSupported returned true for invalid mime type text/plain');\n"
        "if (!MediaSource.canConstructInDedicatedWorker) throw new Error('canConstructInDedicatedWorker missing');\n"
        ...
        "if (!ms.handle) throw new Error('ms.handle missing');\n"
        ...
        "sb.addEventListener('updatestart', function() { updateStarted = true; });\n"
        "var chunk = new Uint8Array([0, 0, 0, 1, 103, 66, 224, 30]);\n"
        "sb.appendBuffer(chunk);\n"
        "if (!sb.updating) throw new Error('sb.updating expected true during appendBuffer');\n"
        "if (!updateStarted) throw new Error('updatestart event expected');\n"
        "if (sb.buffered.length === 0 || sb.buffered.end(0) <= 0) throw new Error('sb.buffered range not updated');\n"
        "sb.abort();\n"
        "if (sb.updating) throw new Error('sb.updating expected false after abort');\n"
        "sb.changeType('video/webm; codecs=\"vp8\"');\n"
        ...
        "if (typeof blobUrl !== 'string' || !blobUrl.startsWith('blob:')) throw new Error('createObjectURL failed for MediaSource');\n"
