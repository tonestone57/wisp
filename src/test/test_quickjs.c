...
START_TEST(test_quickjs_other_apis)
{
    ...
    const char *code =
        ...
        "if (typeof FontFace !== 'function' || typeof FontFaceSet !== 'function') {\n"
        "  throw new Error('FontFace or FontFaceSet constructor missing');\n"
        "}\n"
        "if (!document.fonts || !(document.fonts instanceof FontFaceSet)) {\n"
        "  throw new Error('document.fonts is not FontFaceSet instance');\n"
        "}\n"
        "var ff = new FontFace('TestFont', 'url(test.woff)', { style: 'italic', weight: 'bold' });\n"
        "if (ff.family !== 'TestFont' || ff.style !== 'italic' || ff.weight !== 'bold' || ff.status !== 'loaded') {\n"
        "  throw new Error('FontFace properties invalid');\n"
        "}\n"
        "document.fonts.add(ff);\n"
        "if (!document.fonts.has(ff) || document.fonts.size !== 1) {\n"
        "  throw new Error('FontFaceSet add/has/size failed');\n"
        "}\n"
        "if (document.fonts.check('12px TestFont') !== true) {\n"
        "  throw new Error('FontFaceSet check failed');\n"
        "}\n"
        "document.fonts.delete(ff);\n"
        "if (document.fonts.has(ff) || document.fonts.size !== 0) {\n"
        "  throw new Error('FontFaceSet delete failed');\n"
        "}\n"
