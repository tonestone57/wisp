...
START_TEST(test_grid_layout_3_columns)
{
    ...
    if (child1->width != AUTO) ck_assert_int_gt(child2->x, child1->x + child1->width - 1); /* allowing for 0 gap */
    if (child2->width != AUTO) ck_assert_int_gt(child3->x, child2->x + child2->width - 1);
