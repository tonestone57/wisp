...
START_TEST(test_subsystem_dispatch)
{
    ...
    // Dispatch to JS pool
    wisp_dispatch_js(NULL, test_task, NULL, 0.0f);
    wisp_dispatch_js(NULL, test_task, NULL, 0.0f);

    // Dispatch to Raster pool
    wisp_dispatch_raster(NULL, test_task, NULL, 0.0f);
...
START_TEST(test_subsystem_priority)
{
    ...
    wisp_dispatch_js(NULL, blocker_task, NULL, 1.0f);

    /* 2. Dispatch tasks with different priorities.
     * They will be queued because the worker is blocked. */
    wisp_dispatch_js(NULL, priority_task, (void*)(intptr_t)1, 0.1f); /* Low */
    wisp_dispatch_js(NULL, priority_task, (void*)(intptr_t)3, 0.9f); /* High */
    wisp_dispatch_js(NULL, priority_task, (void*)(intptr_t)2, 0.5f); /* Mid */
