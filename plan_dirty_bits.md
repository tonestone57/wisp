...
*   **Layout Pass**: If a node has `CHILD_DIRTY` but its calculated constraints match its previous layout run, down-tree processing can be skipped for any child that lacks its own `DIRTY` bits.
...
Wisp has successfully transitioned from a union-based dirty region system to a **Fixed-Tile Redraw** strategy as the core architectural standard for all frontends.

### Key Architectural Decisions:
1.  **Scale-Aware Fixed Tile Size**:
    - **Standard DPI**: 256x256.
    - **High-DPI / 4K**: 512x512.
    - **Zero Fragmentation**: Uses a fixed-block memory pool within the arena allocator.
    - **Cache Locality**: Serves as a "surrogate SIMD" for older processors (like i586).
2.  **Redraw Loop**:
    - The engine iterates through dirty tiles, clips the plotter context to the tile boundary, and executes the display list.
    - **Global Redraw Threshold**: If >70% of tiles are dirty, the engine falls back to a full-screen redraw.
3.  **Parallel Rendering**: Tiled independence allows for future multi-threaded rasterization.
