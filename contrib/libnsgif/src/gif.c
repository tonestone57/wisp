 ...
 */
static nsgif_error nsgif__initialise_sprite(struct nsgif *gif, uint32_t width, uint32_t height)
{
    /* Already allocated? */
    if (gif->frame_image) {
        return NSGIF_OK;
 ...
 */
static inline void nsgif__bitmap_set_opaque(const struct nsgif *gif, const struct nsgif_frame *frame)
{
...
static void nsgif__record_frame(struct nsgif *gif, const uint32_t *bitmap)
{
    ...
    if (gif->decoded_frame == NSGIF_FRAME_INVALID || gif->decoded_frame == gif->prev_index) {
    ...
    if (gif->prev_frame == NULL) {
        prev_frame = realloc(gif->prev_frame, width * height * pixel_bytes);
        if (prev_frame == NULL) {
            return;
        }
    } else {
...
static nsgif_error nsgif__recover_frame(const struct nsgif *gif, uint32_t *bitmap)
{
 ...
 * \return true if there is a row to process, false at the end of the frame.
 */
static inline bool nsgif__next_row(uint32_t interlace, uint32_t height, uint32_t *y, uint8_t *step)
{
    if (!interlace) {
        return (++*y != height);
    } else {
 ...
 *         dimension.
 */
static inline uint32_t gif__clip(uint32_t frame_off, uint32_t frame_dim, uint32_t image_ext)
{
 ...
 */
static inline void gif__jump_data(uint32_t *skip, uint32_t *available, const uint8_t **pos)
{
...
static nsgif_error nsgif__decode_complex(struct nsgif *gif, uint32_t width, uint32_t height, uint32_t offset_x,
    uint32_t offset_y, uint32_t interlace, const uint8_t *data, uint32_t transparency_index,
    uint32_t *restrict frame_data, uint32_t *restrict colour_table)
{
    lzw_result res;
    ...
    res = lzw_decode_init(gif->lzw_ctx, data[0], gif->buf, gif->buf_len, data + 1 - gif->buf);
    if (res != LZW_OK) {
    ...
    do {
        ...
        frame_scanline = frame_data + offset_x + (y + offset_y) * gif->rowspan;
        ...
        while (x > 0) {
            ...
            while (available == 0) {
                ...
                res = lzw_decode(gif->lzw_ctx, &uncompressed, &available);
            ...
            if (transparency_index > 0xFF) {
                while (row_available-- > 0) {
                    *frame_scanline++ = colour_table[*uncompressed++];
                }
            } else {
                while (row_available-- > 0) {
...
static nsgif_error nsgif__decode_simple(struct nsgif *gif, uint32_t height, uint32_t offset_y, const uint8_t *data,
    uint32_t transparency_index, uint32_t *restrict frame_data, uint32_t *restrict colour_table)
{
    uint32_t pixels;
    uint32_t written = 0;
    nsgif_error ret = NSGIF_OK;
    ...
    /* Initialise the LZW decoding */
    res = lzw_decode_init_map(
        gif->lzw_ctx, data[0], transparency_index, colour_table, gif->buf, gif->buf_len, data + 1 - gif->buf);
    if (res != LZW_OK) {
        return nsgif__error_from_lzw(res);
    }
    ...
    while (pixels > 0) {
        res = lzw_decode_map(gif->lzw_ctx, frame_data, pixels, &written);
        pixels -= written;
        frame_data += written;
        if (res != LZW_OK) {
...
nsgif__decode(struct nsgif *gif, struct nsgif_frame *frame, const uint8_t *data, uint32_t *restrict frame_data)
{
    nsgif_error ret;
    ...
    if (frame->info.interlaced == false && offset_x == 0 && width == gif->info.width && width == gif->rowspan) {
        ret = nsgif__decode_simple(gif, height, offset_y, data, transparency_index, frame_data, colour_table);
    } else {
        ret = nsgif__decode_complex(gif, width, height, offset_x, offset_y, frame->info.interlaced, data,
            transparency_index, frame_data, colour_table);
    }
 ...
 */
static void nsgif__restore_bg(struct nsgif *gif, struct nsgif_frame *frame, uint32_t *bitmap)
{
    size_t pixel_bytes = sizeof(*bitmap);
    ...
    if (frame == NULL) {
        ...
        memset(bitmap, NSGIF_TRANSPARENT_COLOUR, width * height * pixel_bytes);
    } else {
        uint32_t width = frame->info.rect.x1 - frame->info.rect.x0;
        uint32_t height = frame->info.rect.y1 - frame->info.rect.y0;
        uint32_t offset_x = frame->info.rect.x0;
        ...
        if (frame->info.display == false || frame->info.rect.x0 >= gif->info.width ||
        ...
        if (frame->info.transparency) {
            for (uint32_t y = 0; y < height; y++) {
                uint32_t *scanline = bitmap + offset_x + (offset_y + y) * gif->info.width;
                memset(scanline, NSGIF_TRANSPARENT_COLOUR, width * pixel_bytes);
            }
        } else {
            for (uint32_t y = 0; y < height; y++) {
                uint32_t *scanline = bitmap + offset_x + (offset_y + y) * gif->info.width;
...
static nsgif_error
nsgif__update_bitmap(struct nsgif *gif, struct nsgif_frame *frame, const uint8_t *data, uint32_t frame_idx)
{
    nsgif_error ret;
    uint32_t *bitmap;
 ...
 */
static nsgif_error nsgif__parse_extension_graphic_control(struct nsgif_frame *frame, const uint8_t *data, size_t len)
{
    enum {
    ...
    frame->redraw_required = frame->info.disposal == NSGIF_DISPOSAL_RESTORE_BG ||
        frame->info.disposal == NSGIF_DISPOSAL_RESTORE_PREV;
 ...
 *         NSGIF_OK for success.
 */
static nsgif_error nsgif__parse_extension_application(struct nsgif *gif, const uint8_t *data, size_t len)
{
    ...
    if (nsgif__app_ext_is_loop_count(data, len)) {
        ...
        if ((data[13] == EXT_LOOP_COUNT_SUB_BLOCK_SIZE) && (data[14] == EXT_LOOP_COUNT_SUB_BLOCK_ID)) {
 ...
 */
static nsgif_error
nsgif__parse_frame_extensions(struct nsgif *gif, struct nsgif_frame *frame, const uint8_t **pos, bool decode)
{
    enum {
    ...
    while (nsgif_bytes > 0 && nsgif_data[0] == GIF_EXT_INTRODUCER) {
        ...
        case GIF_EXT_GRAPHIC_CONTROL:
            if (decode) {
                ret = nsgif__parse_extension_graphic_control(frame, nsgif_data, nsgif_bytes);
                if (ret != NSGIF_OK) {
                    return ret;
                }
            }
        ...
        case GIF_EXT_APPLICATION:
            if (decode) {
                ret = nsgif__parse_extension_application(gif, nsgif_data, nsgif_bytes);
                if (ret != NSGIF_OK) {
                    return ret;
                }
            }
 ...
 */
static nsgif_error
nsgif__parse_image_descriptor(struct nsgif *gif, struct nsgif_frame *frame, const uint8_t **pos, bool decode)
{
...
static void nsgif__colour_table_decode(uint32_t colour_table[NSGIF_MAX_COLOURS],
    const struct nsgif_colour_layout *layout, size_t colour_table_entries, const uint8_t *data)
{
 ...
 * \return NSGIF_OK on success, appropriate error otherwise.
 */
static inline nsgif_error nsgif__colour_table_extract(uint32_t colour_table[NSGIF_MAX_COLOURS],
    const struct nsgif_colour_layout *layout, size_t colour_table_entries, const uint8_t *data, size_t data_len,
    size_t *used, bool decode)
{
    ...
    if (decode) {
        nsgif__colour_table_decode(colour_table, layout, colour_table_entries, data);
    }
 ...
 */
static nsgif_error
nsgif__parse_colour_table(struct nsgif *gif, struct nsgif_frame *frame, const uint8_t **pos, bool decode)
{
    nsgif_error ret;
    const uint8_t *data = *pos;
    size_t len = gif->buf + gif->buf_len - data;
    ...
    ret = nsgif__colour_table_extract(gif->local_colour_table, &gif->colour_layout,
        2 << (frame->flags & NSGIF_COLOUR_TABLE_SIZE_MASK), data, len, &used_bytes, decode);
    if (ret != NSGIF_OK) {
        return ret;
    }
    *pos += used_bytes;
 ...
 */
static nsgif_error
nsgif__parse_image_data(struct nsgif *gif, struct nsgif_frame *frame, const uint8_t **pos, bool decode)
{
    const uint8_t *data = *pos;
    size_t len = gif->buf + gif->buf_len - data;
...
static struct nsgif_frame *nsgif__get_frame(struct nsgif *gif, uint32_t frame_idx)
{
 ...
 * \return NSGIF_OK on success, appropriate error otherwise.
 */
static nsgif_error nsgif__process_frame(struct nsgif *gif, uint32_t frame_idx, bool decode)
{
    nsgif_error ret;
    const uint8_t *pos;
    const uint8_t *end;
...
static struct nsgif_colour_layout nsgif__bitmap_fmt_to_colour_layout(nsgif_bitmap_fmt_t bitmap_fmt)
{
    ...
    case NSGIF_BITMAP_FMT_RGBA8888:
        bitmap_fmt = (le) ? NSGIF_BITMAP_FMT_A8B8G8R8 : NSGIF_BITMAP_FMT_R8G8B8A8;
        break;
    case NSGIF_BITMAP_FMT_BGRA8888:
        bitmap_fmt = (le) ? NSGIF_BITMAP_FMT_A8R8G8B8 : NSGIF_BITMAP_FMT_B8G8R8A8;
        break;
    case NSGIF_BITMAP_FMT_ARGB8888:
        bitmap_fmt = (le) ? NSGIF_BITMAP_FMT_B8G8R8A8 : NSGIF_BITMAP_FMT_A8R8G8B8;
        break;
    case NSGIF_BITMAP_FMT_ABGR8888:
        bitmap_fmt = (le) ? NSGIF_BITMAP_FMT_R8G8B8A8 : NSGIF_BITMAP_FMT_A8B8G8R8;
        break;
    default:
        break;
    }
...
/* exported function documented in nsgif.h */
nsgif_error nsgif_create(const nsgif_bitmap_cb_vt *bitmap_vt, nsgif_bitmap_fmt_t bitmap_fmt, nsgif_t **gif_out)
{
    nsgif_t *gif;
...
/* exported function documented in nsgif.h */
void nsgif_set_frame_delay_behaviour(nsgif_t *gif, uint16_t delay_min, uint16_t delay_default)
{
 ...
 * \return NSGIF_OK on success, appropriate error otherwise.
 */
static nsgif_error nsgif__parse_header(struct nsgif *gif, const uint8_t **pos, bool strict)
{
    const uint8_t *data = *pos;
    size_t len = gif->buf + gif->buf_len - data;
    ...
    if (strict == true) {
        if ((strncmp((const char *)data, "87a", 3) != 0) && (strncmp((const char *)data, "89a", 3) != 0)) {
            return NSGIF_ERR_DATA;
        }
    }
    data += 3;
 ...
 * \return NSGIF_OK on success, appropriate error otherwise.
 */
static nsgif_error nsgif__parse_logical_screen_descriptor(struct nsgif *gif, const uint8_t **pos)
{
    const uint8_t *data = *pos;
    size_t len = gif->buf + gif->buf_len - data;
...
nsgif_error nsgif_data_scan(nsgif_t *gif, size_t size, const uint8_t *data)
{
    ...
    if (gif->global_colour_table[0] == NSGIF_PROCESS_COLOURS) {
        ...
        if (gif->info.global_palette && gif->bg_index < gif->colour_table_size) {
...
static void nsgif__redraw_rect_extend(const nsgif_rect_t *frame, nsgif_rect_t *redraw)
{
...
static uint32_t nsgif__frame_next(const nsgif_t *gif, bool partial, uint32_t frame)
{
    uint32_t frames = partial ? gif->frame_count_partial : gif->info.frame_count;
...
static nsgif_error nsgif__next_displayable_frame(const nsgif_t *gif, uint32_t *frame, uint32_t *delay)
{
    uint32_t next = *frame;
...
/* exported function documented in nsgif.h */
nsgif_error nsgif_frame_prepare(nsgif_t *gif, nsgif_rect_t *area, uint32_t *delay_cs, uint32_t *frame_new)
{
    nsgif_error ret;
    nsgif_rect_t rect = {
        .x1 = 0,
    ...
    if (gif->frame != NSGIF_FRAME_INVALID && gif->frame < gif->info.frame_count &&
    ...
    if (gif->data_complete) {
        ...
        if (gif->info.frame_count == 1) {
            ...
            if (gif->data_complete && frame_next < frame) {
                if (nsgif__animation_complete(gif->loop_count + 1, gif->info.loop_max)) {
                    delay = NSGIF_INFINITE;
                }
            }
        }
...
/* exported function documented in nsgif.h */
nsgif_error nsgif_frame_decode(nsgif_t *gif, uint32_t frame, nsgif_bitmap_t **bitmap)
{
    ...
    } else if (gif->decoded_frame >= frame || gif->decoded_frame == NSGIF_FRAME_INVALID) {
...
/* exported function documented in nsgif.h */
const nsgif_frame_info_t *nsgif_get_frame_info(const nsgif_t *gif, uint32_t frame)
{
    if (frame >= gif->info.frame_count) {
        return NULL;
    }
...
/* exported function documented in nsgif.h */
void nsgif_global_palette(const nsgif_t *gif, uint32_t table[NSGIF_MAX_COLOURS], size_t *entries)
{
...
/* exported function documented in nsgif.h */
bool nsgif_local_palette(const nsgif_t *gif, uint32_t frame, uint32_t table[NSGIF_MAX_COLOURS], size_t *entries)
{
    const nsgif_frame *f;
