...
lzw__read_code(struct lzw_read_ctx *restrict ctx, uint16_t code_size, uint16_t *restrict code_out)
{
    uint32_t code = 0;
    ...
    if (ctx->sb_bit + 24 <= ctx->sb_bit_count) {
        ...
        uint8_t byte = 0;
        uint8_t bits_remaining_0 = (code_size < (8u - current_bit)) ? code_size : (8u - current_bit);
 ...
 */
static inline lzw_result lzw__handle_clear(struct lzw_ctx *ctx, uint16_t *code_out)
{
    uint16_t code;
    ...
    do {
        lzw_result res = lzw__read_code(&ctx->input, ctx->code_size, &code);
        if (res != LZW_OK) {
            return res;
        }
...
lzw_result lzw_decode_init(
    struct lzw_ctx *ctx, uint8_t minimum_code_size, const uint8_t *input_data, size_t input_length, size_t input_pos)
{
...
/* Exported function, documented in lzw.h */
lzw_result lzw_decode_init_map(struct lzw_ctx *ctx, uint8_t minimum_code_size, uint32_t transparency_idx,
    const uint32_t *colour_table, const uint8_t *input_data, size_t input_length, size_t input_pos)
{
    lzw_result res;
    ...
    res = lzw_decode_init(ctx, minimum_code_size, input_data, input_length, input_pos);
    if (res != LZW_OK) {
        return res;
    }
...
typedef uint32_t (*lzw_writer_fn)(struct lzw_ctx *ctx, void *restrict output_data, uint32_t output_length,
    uint32_t output_pos, uint16_t code, uint16_t left);
 ...
 */
static inline lzw_result lzw__decode(struct lzw_ctx *ctx, lzw_writer_fn write_fn, void *restrict output_data,
    uint32_t output_length, uint32_t *restrict output_written)
{
    lzw_result res;
    uint16_t code;
    ...
    if (code == ctx->eoi_code) {
        ...
        lzw__table_add_entry(ctx, (code < size) ? ctx->table[code].first : ctx->prev_code_first);
    ...
    *output_written += write_fn(ctx, output_data, output_length, *output_written, code, ctx->table[code].count);
 ...
 */
static inline uint32_t lzw__write_fn(struct lzw_ctx *ctx, void *restrict output_data, uint32_t output_length,
    uint32_t output_used, uint16_t code, uint16_t left)
{
...
/* Exported function, documented in lzw.h */
lzw_result
lzw_decode(struct lzw_ctx *ctx, const uint8_t *restrict *const restrict output_data, uint32_t *restrict output_written)
{
    ...
    *output_data = ctx->stack_base;
    ...
    if (ctx->output_left != 0) {
        *output_written += lzw__write_fn(
            ctx, ctx->stack_base, output_length, *output_written, ctx->output_code, ctx->output_left);
    }

    while (*output_written != output_length) {
        lzw_result res = lzw__decode(ctx, lzw__write_fn, ctx->stack_base, output_length, output_written);
        if (res != LZW_OK) {
            return res;
        }
    }
 ...
 * \return Number of pixel values written.
 */
static inline uint32_t lzw__map_write_fn(struct lzw_ctx *ctx, void *restrict output_data, uint32_t output_length,
    uint32_t output_used, uint16_t code, uint16_t left)
{
...
/* Exported function, documented in lzw.h */
lzw_result lzw_decode_map(
    struct lzw_ctx *ctx, uint32_t *restrict output_data, uint32_t output_length, uint32_t *restrict output_written)
{
    *output_written = 0;
    ...
    if (ctx->output_left != 0) {
        *output_written += lzw__map_write_fn(
            ctx, output_data, output_length, *output_written, ctx->output_code, ctx->output_left);
    }

    while (*output_written != output_length) {
        lzw_result res = lzw__decode(ctx, lzw__map_write_fn, output_data, output_length, output_written);
        if (res != LZW_OK) {
            return res;
        }
    }
