...
static bool cli__parse_value_int(const char *str, int64_t *i, size_t *pos)
{
    ...
    if (end == str || errno == ERANGE || temp > INT64_MAX || temp < INT64_MIN) {
 ...
 * \return true on success, or false otherwise.
 */
static bool cli__parse_value_enum(const char *str, const struct cli_enum *e, size_t *pos)
{
    str += *pos;
    *pos += strlen(str);
 ...
 * \return true on success, or false otherwise.
 */
static bool cli__parse_value_string(const char *str, const char **s, size_t *pos)
{
    *s = str + *pos;
    *pos += strlen(*s);
    return true;
 ...
 * \return true on success, or false otherwise.
 */
static bool cli__parse_value(const struct cli_table_entry *entry, const char *arg, size_t *pos)
{
    switch (entry->t) {
    case CLI_CMD:
 ...
 */
static bool
cli__parse_argv_value(const struct cli_table_entry *entry, int argc, const char **argv, int arg_pos, size_t *pos)
{
 ...
 */
static const struct cli_table_entry *cli__lookup_short(const struct cli_table *cli, char s)
{
 ...
 * \return true on success, or false otherwise.
 */
static bool cli__handle_arg_value(
    const struct cli_table_entry *entry, int argc, const char **argv, int *arg_pos, size_t pos, char sep)
{
    const char *arg = argv[*arg_pos];
    size_t orig_pos;
    bool ret;
...
static inline bool cli__is_negative(const char *arg)
{
    ...
    return cli__parse_value_int(arg, &i, &pos) && pos == strlen(arg) && i < 0;
}
 ...
 * \return true on success, or false otherwise.
 */
static bool cli__parse_positional_entry(struct cli_ctx *ctx, const struct cli_table_entry *entry, const char *arg)
{
    size_t pos = 0;
    bool ret;
    ...
    for (size_t i = 0; i < cli->count; i++) {
        if (cli__entry_is_positional(&cli->entries[i])) {
            if (positional == ctx->pos_count) {
                return cli__parse_positional_entry(ctx, &cli->entries[i], arg);
            }
 ...
 * \return true on success, or false otherwise.
 */
static bool cli__parse_short(struct cli_ctx *ctx, int argc, const char **argv, int *arg_pos)
{
    const char *arg = argv[*arg_pos];
    size_t pos = 1;
    ...
    while (arg[pos] != '\0') {
        ...
        if (entry->t == CLI_BOOL) {
            *entry->v.b = true;
            ...
            return cli__handle_arg_value(entry, argc, argv, arg_pos, pos + 1, '\0');
        }
 ...
 * \return Client CLI spec entry on success, or NULL otherwise.
 */
static const struct cli_table_entry *cli__lookup_long(const struct cli_table *cli, const char *arg, size_t *pos)
{
    arg += *pos;
    ...
    for (size_t i = 0; i < cli->count; i++) {
        if (cli__entry_is_positional(&cli->entries[i]) == false) {
            ...
            if (strncmp(name, arg, name_len) == 0) {
                if (arg[name_len] != '\0' && arg[name_len] != '=') {
                    continue;
                }
                *pos += name_len;
                return &cli->entries[i];
 ...
 * \return true on success, or false otherwise.
 */
static bool cli__parse_long(struct cli_ctx *ctx, int argc, const char **argv, int *arg_pos)
{
    const struct cli_table_entry *entry;
    const char *arg = argv[*arg_pos];
    size_t pos = 2;
    ...
    if (entry->t == CLI_BOOL) {
        ...
        *entry->v.b = true;
        ...
        ret = cli__handle_arg_value(entry, argc, argv, arg_pos, pos, '=');
        if (ret != true) {
            return ret;
        }
    }
 ...
 */
static void cli__count(
    const struct cli_table *cli, size_t *count, size_t *pcount, size_t *max_len, size_t *pmax_len, size_t *phas_desc)
{
    if (count != NULL)
        *count = 0;
    if (pcount != NULL)
...
static void cli__print_wrapping_string(const char *str, size_t indent)
{
    ...
    while (*str != '\0') {
        ...
        if (word_len <= space || space == avail) {
            fprintf(stderr, "%*.*s", (int)word_len, (int)word_len, str);
            str += word_len;
            if (word_len <= space) {
                space -= word_len;
            }
 ...
 * \param[in] indent  The number of spaces to pad the left margin with.
 */
static void cli__print_description(const struct cli_table_entry *entry, size_t indent)
{
    if (entry->d != NULL) {
    ...
    if (entry->t == CLI_ENUM) {
        ...
        for (const struct cli_str_val *e = entry->v.e.desc; e->str != NULL; e++) {
        ...
        for (const struct cli_str_val *e = entry->v.e.desc; e->str != NULL; e++) {
            ...
            if (e->d == NULL || e->d[0] == '\0') {
                ...
                fprintf(stderr, "%-*s - ", (int)(max_len), e->str);
                cli__print_wrapping_string(e->d, 8 + max_len + 3);
                fprintf(stderr, "\n");
            }
        }
    }
...
void cli_help(const struct cli_table *cli, const char *prog_name)
{
    ...
    if (pcount > 0) {
        for (size_t i = 0; i < cli->count; i++) {
            if (cli__entry_is_positional(&cli->entries[i])) {
                const char *punctuation = (required == cli->min_positional) ? " [" : " ";

                if (cli->entries[i].t == CLI_CMD) {
                    fprintf(stderr, "%s%s", punctuation, cli->entries[i].l);
                } else {
                    fprintf(stderr, "%s<%s>", punctuation, cli->entries[i].l);
                }
                required++;
    ...
    if (phas_desc > 0) {
        ...
        for (size_t i = 0; i < cli->count; i++) {
            ...
            if (cli__entry_is_positional(entry)) {
                fprintf(stderr, "  %*.*s  ", (int)pmax_len, (int)pmax_len, entry->l);
    ...
    if (count > 0) {
        ...
        for (size_t i = 0; i < cli->count; i++) {
            ...
            fprintf(stderr, "  --%s %s%*.s  ", entry->l, type_str, (int)(max_len - arg_len - type_len), "");
