...
static const struct cli_table_entry cli_entries[] = {
    ...
    {.s = 'i', .l = "info", .t = CLI_BOOL, .v.b = &nsgif_options.info, .d = "Dump GIF info to stdout."},
    {.s = 'l',
        .l = "loops",
        .t = CLI_UINT,
    ...
    {.s = 'p', .l = "palette", .t = CLI_BOOL, .v.b = &nsgif_options.palette, .d = "Save palette images."},
    {.s = 'V',
        .l = "version",
        .t = CLI_BOOL,
        .no_pos = true,
    ...
    {.p = true, .l = "FILE", .t = CLI_STRING, .v.s = &nsgif_options.file, .d = "Path to GIF file to load."},
};
...
static bool save_palette(const char *img_filename, const char *palette_filename,
    const uint32_t palette[NSGIF_MAX_COLOURS], size_t used_entries)
{
    enum {
        SIZE = 32,
        COUNT = 16,
    ...
    for (int y = 0; y < size; y++) {
        for (int x = 0; x < size; x++) {
            if (x % SIZE == 0 || y % SIZE == 0) {
                ...
                fprintf(f, "%u %u %u ", entry[0], entry[1], entry[2]);
            }
        }
...
static bool save_local_palette(const nsgif_t *gif, uint32_t frame)
{
    ...
    snprintf(filename, sizeof(filename), "local-palette-%" PRIu32 ".ppm", frame);
        ...
        if (err != NSGIF_OK) {
            ...
            for (uint32_t y = 0; y != info->height; y++) {
                for (uint32_t x = 0; x != info->width; x++) {
                    ...
                    fprintf(ppm, "%u %u %u ", image[z], image[z + 1], image[z + 2]);
                }
                fprintf(ppm, "\n");
            }
        }
