...
hubbub_error handle_after_head(hubbub_treebuilder *treebuilder, const hubbub_token *token)
{
    ...
    case HUBBUB_TOKEN_START_TAG: {
        ...
        if (type == HTML) {
        ...
        } else if (type == BASE || type == LINK || type == META || type == NOFRAMES || type == SCRIPT ||
            ...
            err = element_stack_push(treebuilder, HUBBUB_NS_HTML, HEAD, treebuilder->context.head_element);
            if (err != HUBBUB_OK)
                return err;
            ...
            element_stack_remove(treebuilder, index, &ns, &otype, &node);
