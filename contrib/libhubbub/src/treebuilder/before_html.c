...
hubbub_error handle_before_html(hubbub_treebuilder *treebuilder, const hubbub_token *token)
{
    ...
    if (handled || err == HUBBUB_REPROCESS) {
        ...
        if (err == HUBBUB_REPROCESS) {
            ...
            e = treebuilder->tree_handler->create_element(treebuilder->tree_handler->ctx, &tag, &html);
        } else {
            e = treebuilder->tree_handler->create_element(treebuilder->tree_handler->ctx, &token->data.tag, &html);
        }
        ...
        e = treebuilder->tree_handler->append_child(
            treebuilder->tree_handler->ctx, treebuilder->context.document, html, &appended);

        treebuilder->tree_handler->unref_node(treebuilder->tree_handler->ctx, html);
