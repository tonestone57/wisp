...
static hubbub_error process_meta_in_head(hubbub_treebuilder *treebuilder, const hubbub_token *token)
{
    ...
    for (i = 0; i < token->data.tag.n_attributes; i++) {
        ...
        if (hubbub_string_match(attr->name.ptr, attr->name.len, (const uint8_t *)"charset", SLEN("charset")) == true) {
            /* Extract charset */
            charset_enc = parserutils_charset_mibenum_from_name((const char *)attr->value.ptr, attr->value.len);
        } else if (hubbub_string_match(attr->name.ptr, attr->name.len, (const uint8_t *)"content", SLEN("content")) ==
            true) {
            /* Extract charset from Content-Type */
            content_type_enc = hubbub_charset_parse_content(attr->value.ptr, attr->value.len);
        }
    }
    ...
    if (charset_enc != 0) {
        ...
        err = treebuilder->tree_handler->encoding_change(treebuilder->tree_handler->ctx, name);
    }
