...
static const struct dom_element_protected_vtable _protect_vtable = {
    {DOM_NODE_PROTECT_VTABLE_HTML_TABLE_ELEMENT}, DOM_HTML_TABLE_ELEMENT_PROTECT_VTABLE};
...
#define SIMPLE_GET(attr)                                                                                               \
    dom_exception dom_html_table_element_get_##attr(dom_html_table_element *element, dom_string **attr)                \
    {                                                                                                                  \
        dom_exception ret;                                                                                             \
...
#define SIMPLE_SET(attr)                                                                                               \
    dom_exception dom_html_table_element_set_##attr(dom_html_table_element *element, dom_string *attr)                 \
    {                                                                                                                  \
        dom_exception ret;                                                                                             \
        dom_string *_memo_##attr;                                                                                      \
                                                                                                                       \
 ...
 */
dom_exception
dom_html_table_element_get_caption(dom_html_table_element *table, dom_html_table_caption_element **caption)
{
    ...
    for (node_tmp = node_tmp->first_child; node_tmp != NULL; node_tmp = node_tmp->next) {
        if ((node_tmp->type == DOM_ELEMENT_NODE) &&
            dom_string_caseless_isequal(doc->elements[DOM_HTML_ELEMENT_TYPE_CAPTION], node_tmp->name)) {
            break;
        }
    }
...
dom_exception dom_html_table_element_set_caption(dom_html_table_element *table, dom_html_table_caption_element *caption)
{
    ...
    if (!dom_string_caseless_isequal(doc->elements[DOM_HTML_ELEMENT_TYPE_CAPTION], check_node->name)) {
        return DOM_HIERARCHY_REQUEST_ERR;
    }
...
dom_exception dom_html_table_element_get_t_head(dom_html_table_element *table, dom_html_table_section_element **t_head)
{
    ...
    for (node_tmp = node_tmp->first_child; node_tmp != NULL; node_tmp = node_tmp->next) {
        if ((node_tmp->type == DOM_ELEMENT_NODE) &&
            dom_string_caseless_isequal(doc->elements[DOM_HTML_ELEMENT_TYPE_THEAD], node_tmp->name)) {
            break;
        }
    }
...
dom_exception dom_html_table_element_get_t_foot(dom_html_table_element *table, dom_html_table_section_element **t_foot)
{
    ...
    for (node_tmp = node_tmp->first_child; node_tmp != NULL; node_tmp = node_tmp->next) {
        if ((node_tmp->type == DOM_ELEMENT_NODE) &&
            dom_string_caseless_isequal(doc->elements[DOM_HTML_ELEMENT_TYPE_TFOOT], node_tmp->name)) {
            break;
        }
    }
...
{
    dom_node_internal *check_node = ((dom_node_internal *)t_foot); /*< temporary node to check for
                                                      raised exceptions */
    dom_html_document *doc = (dom_html_document *)(((dom_node_internal *)table)->owner);
    dom_exception exp;
    dom_node *new_t_foot;
    ...
    if (!dom_string_caseless_isequal(doc->elements[DOM_HTML_ELEMENT_TYPE_TFOOT], check_node->name)) {
        return DOM_HIERARCHY_REQUEST_ERR;
    }
...
static bool table_rows_callback(struct dom_node_internal *node, void *ctx)
{
    ...
    if (node->type == DOM_ELEMENT_NODE &&
        dom_string_caseless_isequal(node->name, doc->elements[DOM_HTML_ELEMENT_TYPE_TR])) {
        return true;
    }
    return false;
}
...
dom_exception dom_html_table_element_get_rows(dom_html_table_element *element, dom_html_collection **rows)
{
    ...
    return _dom_html_collection_create(doc, (dom_node_internal *)element, table_rows_callback, (void *)doc, rows);
}
...
static bool table_t_bodies_callback(struct dom_node_internal *node, void *ctx)
{
    dom_html_document *doc = ctx;
    if (node->type == DOM_ELEMENT_NODE &&
        dom_string_caseless_isequal(node->name, doc->elements[DOM_HTML_ELEMENT_TYPE_TBODY])) {
        return true;
    }
    return false;
}
...
dom_exception dom_html_table_element_get_t_bodies(dom_html_table_element *element, dom_html_collection **t_bodies)
{
    ...
    return _dom_html_collection_create(
        doc, (dom_node_internal *)element, table_t_bodies_callback, (void *)doc, t_bodies);
}
...
{
    dom_exception exp;
    if ((exp = dom_html_table_element_get_caption(element, (dom_html_table_caption_element **)caption)) != DOM_NO_ERR) {
    ...
    if ((*caption) == NULL) {
        ...
        struct dom_html_element_create_params params = {.type = DOM_HTML_ELEMENT_TYPE_CAPTION,
            .doc = doc,
        ...
        exp = _dom_html_table_caption_element_create(&params, (dom_html_table_caption_element **)caption);
        if (exp != DOM_NO_ERR) {
            dom_node_unref(*caption);
            return exp;
        }
...
{
    dom_exception exp;
    exp = dom_html_table_element_get_t_foot(element, (dom_html_table_section_element **)t_foot);
    if (exp != DOM_NO_ERR)
        return exp;
    ...
    if ((*t_foot) == NULL) {
        ...
        struct dom_html_element_create_params params = {.type = DOM_HTML_ELEMENT_TYPE_TFOOT,
            .doc = doc,
            .name = doc->elements[DOM_HTML_ELEMENT_TYPE_TFOOT],
            .namespace = ((dom_node_internal *)element)->namespace,
            .prefix = ((dom_node_internal *)element)->prefix};

        exp = _dom_html_table_section_element_create(&params, (dom_html_table_section_element **)t_foot);
        if (exp != DOM_NO_ERR) {
...
{
    dom_exception exp;
    exp = dom_html_table_element_get_t_head(element, (dom_html_table_section_element **)t_head);
    if (exp != DOM_NO_ERR) {
    ...
    if ((*t_head) == NULL) {
        ...
        struct dom_html_element_create_params params = {.type = DOM_HTML_ELEMENT_TYPE_THEAD,
            .doc = doc,
            .name = doc->elements[DOM_HTML_ELEMENT_TYPE_THEAD],
            .namespace = ((dom_node_internal *)element)->namespace,
            .prefix = ((dom_node_internal *)element)->prefix};

        exp = _dom_html_table_section_element_create(&params, (dom_html_table_section_element **)t_head);
        if (exp != DOM_NO_ERR) {
            dom_node_unref(*t_head);
            return exp;
        }
...
dom_html_table_element_create_t_body(dom_html_table_element *element, dom_html_table_section_element **t_body)
{
    ...
    if (len == 0) {
        ...
        struct dom_html_element_create_params params = {.type = DOM_HTML_ELEMENT_TYPE_TBODY,
            .doc = doc,
        ...
        exp = dom_html_collection_item(t_bodies, 0, (dom_node **)t_body);
    }
    dom_html_collection_unref(t_bodies);
    return exp;
}
...
dom_html_table_element_insert_row(dom_html_table_element *element, int32_t index, dom_html_element **row_out)
{
    ...
    if (index > (int32_t)len || index < -1) {
        ...
        struct dom_html_element_create_params params = {.type = DOM_HTML_ELEMENT_TYPE_TR,
            .doc = doc,
        ...
        exp = dom_node_append_child(new_body, (dom_html_element *)row, &new_row);
        ...
        if (window_len + section_len > (uint32_t)index || window_len + section_len == len) {
            exp = dom_html_table_section_element_insert_row(t_head, index - window_len, &row);
            dom_node_unref(t_head);
            if (exp == DOM_NO_ERR) {
                *row_out = row;
            }
        ...
        for (n = n->first_child; n != NULL; n = n->next) {
            if ((n->type == DOM_ELEMENT_NODE) &&
                dom_string_caseless_isequal(doc->elements[DOM_HTML_ELEMENT_TYPE_TBODY], n->name)) {
                ...
                if (window_len + section_len > (uint32_t)index || window_len + section_len == len) {
                    exp = dom_html_table_section_element_insert_row(
                        (dom_html_table_section_element *)n, index - window_len, &row);
                    if (exp == DOM_NO_ERR) {
                        *row_out = row;
                    }
                    return exp;
        ...
        if (window_len + section_len > (uint32_t)index || window_len + section_len == len) {
            exp = dom_html_table_section_element_insert_row(t_foot, index - window_len, &row);
            dom_node_unref(t_foot);
            if (exp == DOM_NO_ERR) {
                *row_out = row;
            }
...
dom_exception dom_html_table_element_delete_row(dom_html_table_element *element, int32_t index)
{
    ...
    if (index >= (int32_t)len || index < -1 || len == 0) {
        ...
        if (window_len + section_len > (uint32_t)index) {
            exp = dom_html_table_section_element_delete_row(t_head, index - window_len);
            dom_node_unref(t_head);
            return exp;
        }
        dom_node_unref(t_head);
            ...
            if ((n->type == DOM_ELEMENT_NODE) &&
                ...
                if (window_len + section_len > (uint32_t)index) {
                    return dom_html_table_section_element_delete_row(
                        (dom_html_table_section_element *)n, index - window_len);
                }
                window_len += section_len;
            }
        }
        ...
        if (window_len + section_len > (uint32_t)index) {
            exp = dom_html_table_section_element_delete_row(t_foot, index - window_len);
            dom_node_unref(t_foot);
            return exp;
        }
        return DOM_INDEX_SIZE_ERR;
