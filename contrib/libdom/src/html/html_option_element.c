...
static struct dom_element_protected_vtable _protect_vtable = {
    {DOM_NODE_PROTECT_VTABLE_HTML_OPTION_ELEMENT}, DOM_HTML_OPTION_ELEMENT_PROTECT_VTABLE};
...
dom_exception dom_html_option_element_get_form(dom_html_option_element *option, dom_html_form_element **form)
{
    ...
    while (select != NULL) {
        if (select->type == DOM_ELEMENT_NODE &&
            dom_string_caseless_isequal(select->name, doc->elements[DOM_HTML_ELEMENT_TYPE_SELECT]))
            break;
    ...
    if (select != NULL) {
        return dom_html_select_element_get_form((dom_html_select_element *)select, form);
    }
 ...
 */
static dom_exception dom_html_option_element_get_text_node(dom_node_internal *n, dom_string **text)
{
    ...
    *text = NULL;
    ...
    for (n = n->first_child; n != NULL; n = n->next) {
        ...
        if (n->type == DOM_ELEMENT_NODE) {
            ...
            if (exc != DOM_NO_ERR)
                return exc;
            if (dom_string_caseless_isequal(node_name, owner->script_string)) {
                exc = dom_node_get_namespace(n, &node_ns);
                if (exc != DOM_NO_ERR) {
                    dom_string_unref(node_name);
                    return exc;
                }
                if (dom_string_caseless_isequal(node_ns, dom_namespaces[DOM_NAMESPACE_HTML]) ||
                    dom_string_caseless_isequal(node_ns, dom_namespaces[DOM_NAMESPACE_SVG])) {
                    dom_string_unref(node_name);
            ...
            dom_html_option_element_get_text_node(n, (str == NULL) ? &str : &ret);
        } else {
    ...
    if (str != NULL) {
        dom_string_whitespace_op(str, DOM_WHITESPACE_STRIP_COLLAPSE, text);
        dom_string_unref(str);
    }
...
{
    return dom_html_option_element_get_text_node((dom_node_internal *)option, text);
}
...
dom_exception dom_html_option_element_get_label(dom_html_option_element *option, dom_string **label)
{
    ...
    return dom_element_get_attribute(option, doc->memoised[hds_label], label);
}
...
dom_exception dom_html_option_element_set_label(dom_html_option_element *option, dom_string *label)
{
    ...
    return dom_element_set_attribute(option, doc->memoised[hds_label], label);
}
...
{
    return dom_html_element_get_bool_property(&ele->base, "selected", SLEN("selected"), selected);
}
...
{
    return dom_html_element_set_bool_property(&ele->base, "selected", SLEN("selected"), selected);
}
...
dom_exception dom_html_option_element_get_value(dom_html_option_element *option, dom_string **value)
{
    ...
    err = dom_element_has_attribute(option, doc->memoised[hds_value], &has_value);
    if (err != DOM_NO_ERR)
        return err;

    if (has_value) {
        return dom_element_get_attribute(option, doc->memoised[hds_value], value);
    }
...
dom_exception dom_html_option_element_set_value(dom_html_option_element *option, dom_string *value)
{
    ...
    return dom_element_set_attribute(option, doc->memoised[hds_value], value);
}
