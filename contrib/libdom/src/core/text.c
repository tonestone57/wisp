...
static const struct dom_node_protect_vtable text_protect_vtable = {DOM_TEXT_PROTECT_VTABLE};
...
static dom_exception walk_logic_adjacent_text_in_order(
    dom_node_internal *node, walk_operation opt, walk_order order, dom_string **ret, bool *cont);
/* Walk the logic-adjacent text */
static dom_exception walk_logic_adjacent_text(dom_text *text, walk_operation opt, dom_string **ret);
 ...
 */
dom_exception
_dom_text_initialise(dom_text *text, dom_document *doc, dom_node_type type, dom_string *name, dom_string *value)
{
    dom_exception err;

    /* Initialise the base class */
    err = _dom_characterdata_initialise(&text->base, doc, type, name, value);
    if (err != DOM_NO_ERR)
        return err;
...
dom_exception _dom_text_split_text(dom_text *text, uint32_t offset, dom_text **result)
{
    ...
    err = dom_characterdata_substring_data(&text->base, offset, len - offset, &value);
    if (err != DOM_NO_ERR) {
        return err;
    }
 ...
 * \return DOM_NO_ERR on success, appropriate dom_exception on failure.
 */
dom_exception walk_logic_adjacent_text(dom_text *text, walk_operation opt, dom_string **ret)
{
