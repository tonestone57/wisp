...
static inline uint8_t get_counter_set_bits(const css_computed_style *style)
{
	uint32_t bits = style->i.bits[COUNTER_SET_INDEX];
	bits &= COUNTER_SET_MASK;
	bits >>= COUNTER_SET_SHIFT;

	/* 2bits: tt : type */
	return (bits & 0x3);
}
static inline uint8_t get_counter_set(const css_computed_style *style, const
		css_computed_counter **counter_arr)
{
	uint32_t bits = style->i.bits[COUNTER_SET_INDEX];
	bits &= COUNTER_SET_MASK;
...
static inline uint8_t get_flex_basis(const css_computed_style *style,
		...
		int32_t *integer)
{
	...
	/* 2bits: tt : type */
	if ((bits & 0x3) == CSS_GRID_LINE_SET || (bits & 0x3) ==
			CSS_GRID_LINE_SPAN) {
...
static inline uint8_t get_grid_template_columns(const css_computed_style
		*style, css_computed_grid_track **grid_track_arr)
{
	uint32_t bits = style->i.bits[GRID_TEMPLATE_COLUMNS_INDEX];
	bits &= GRID_TEMPLATE_COLUMNS_MASK;
	bits >>= GRID_TEMPLATE_COLUMNS_SHIFT;
...
static inline uint8_t get_grid_template_rows(const css_computed_style *style,
		css_computed_grid_track **grid_track_arr)
{
	uint32_t bits = style->i.bits[GRID_TEMPLATE_ROWS_INDEX];
	bits &= GRID_TEMPLATE_ROWS_MASK;
	bits >>= GRID_TEMPLATE_ROWS_SHIFT;
...
static inline uint8_t get_tab_size_bits(const css_computed_style *style)
{
...
static inline uint8_t get_tab_size(const css_computed_style *style, int32_t
		*integer)
{
	uint32_t bits = style->i.bits[TAB_SIZE_INDEX];
	bits &= TAB_SIZE_MASK;
	...
	/* 2bits: tt : type */
	if ((bits & 0x3) == CSS_TAB_SIZE_SET) {
		*integer = style->i.tab_size;
	}
