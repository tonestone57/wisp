...
static parserutils_vector *create_token_vector(lwc_string *val)
{
    parserutils_vector *v;
    css_token t;
    memset(&t, 0, sizeof(t));
    t.type = CSS_TOKEN_IDENT;
    t.idata = lwc_string_ref(val);
    parserutils_vector_create(sizeof(css_token), 8, &v);
    parserutils_vector_append(v, &t);
    return v;
}
...
int main(void)
{
    ...
    lwc_string *val1_str, *val2_str, *val3_str;
    parserutils_vector *val1, *val2, *val3;
    ...
    assert(lwc_intern_string("blue", 4, &val1_str) == lwc_error_ok);
    assert(lwc_intern_string("16px", 4, &val2_str) == lwc_error_ok);
    assert(lwc_intern_string("red", 3, &val3_str) == lwc_error_ok);

    val1 = create_token_vector(val1_str);
    val2 = create_token_vector(val2_str);
    val3 = create_token_vector(val3_str);
    ...
    assert(ctx->count == 2);
    assert(css__variables_ctx_get(ctx, name1) != NULL);
    assert(css__variables_ctx_get(ctx, name2) != NULL);
    ...
    assert(css__variables_ctx_get(clone, name1) != NULL);
    assert(css__variables_ctx_get(clone, name2) != NULL);
    printf("Test 7 (clone): PASS\n");

    /* Test 8: Cloned context is independent — modify original */
    error = css__variables_ctx_set(ctx, name1, val1);
    assert(error == CSS_OK);
    assert(css__variables_ctx_get(ctx, name1) != NULL);     /* original changed */
    assert(css__variables_ctx_get(clone, name1) != NULL);   /* clone unchanged */
    ...
    css__tokens_destroy(val2);
    css__tokens_destroy(val3);

    /* Clean up interned strings */
    lwc_string_unref(name1);
    lwc_string_unref(name2);
    lwc_string_unref(name3);
    lwc_string_unref(val1_str);
    lwc_string_unref(val2_str);
    lwc_string_unref(val3_str);
