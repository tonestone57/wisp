...
void nsws_drawable_paint_blend2d(struct gui_window *gw, HWND hwnd)
{
    ...
    struct blend2d_context bl_wrap = {
        ...
        .native_priv = NULL /* HDC will be set in plotters via global if needed, but win_plotters.text uses plot_hdc */
    };
    ...
    /* win_plotters.text uses global plot_hdc */
    plot_hdc = hdc;
    ...
    SetDIBitsToDevice(hdc, 0, 0, width, height, 0, 0, 0, height, img_data.pixelData, &bmi, DIB_RGB_COLORS);
