...
static gboolean nsgtk_window_scroll_event(GtkWidget *widget, GdkEventScroll *event, gpointer data)
{
    ...
    if (browser_window_scroll_at_point(g->bw, event->x, event->y, deltax, deltay) != true) {
        ...
        if (deltax != 0) {
            ...
            int layout_width = gtk_widget_get_allocated_width(GTK_WIDGET(g->layout));

            if (value > nsgtk_adjustment_get_upper(hscroll) - layout_width) {
                value = nsgtk_adjustment_get_upper(hscroll) - layout_width;
            }
        ...
        if (deltay != 0) {
            ...
            int layout_height = gtk_widget_get_allocated_height(GTK_WIDGET(g->layout));

            if (value > (nsgtk_adjustment_get_upper(vscroll) - layout_height)) {
                value = nsgtk_adjustment_get_upper(vscroll) - layout_height;
            }
...
static gboolean nsgtk_window_keypress_event(GtkWidget *widget, GdkEventKey *event, gpointer data)
{
    ...
    case GDK_KEY(End):
    case GDK_KEY(KP_End):
        value = nsgtk_adjustment_get_upper(vscroll) - gtk_widget_get_allocated_height(GTK_WIDGET(g->layout));
    ...
    case GDK_KEY(KP_Right):
        ...
        if (value > nsgtk_adjustment_get_upper(hscroll) - gtk_widget_get_allocated_width(GTK_WIDGET(g->layout)))
            value = nsgtk_adjustment_get_upper(hscroll) - gtk_widget_get_allocated_width(GTK_WIDGET(g->layout));
    ...
    case GDK_KEY(KP_Down):
        ...
        if (value > nsgtk_adjustment_get_upper(vscroll) - gtk_widget_get_allocated_height(GTK_WIDGET(g->layout)))
            value = nsgtk_adjustment_get_upper(vscroll) - gtk_widget_get_allocated_height(GTK_WIDGET(g->layout));
