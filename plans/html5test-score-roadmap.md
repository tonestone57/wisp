...
| :--- | :---: | :---: | :---: | :---: | :--- |
| **Parsing & Doctype** (`parsing`) | 3 | 5 | 2 | High Pass | HTML5 fragment tokenizer (`parsing.tokenizer`) & tree building (`parsing.tree`), missing: `parsing.doctype` (`document.compatMode`) (2 pts) |
...
| **Forms & Input** (`form`) | 65 | 66 | 1 | High Pass | `HTMLInputElement` types (`date`, `color`, `range`, `number`, `time`, `datetime-local`, `url`, `email`), `valueAsDate`, `valueAsNumber`, missing: legacy `input type=datetime` (1 pt) |
...
| **Vector Graphics** (`svg`) | 3 | 4 | 1 | High Pass | Inline SVG, `SVGElement` prototypes, `SVGForeignObjectElement`, `SVGFEColorMatrixElement`, missing: `svg.image` (embedded SVG `<image>`) (1 pt) |
...
| **Communication** (`communication`) | 35 | 40 | 5 | High Pass | `EventSource`, `navigator.sendBeacon`, `fetch`, `XMLHttpRequest.upload`, `XMLHttpRequest` (responseType text/document/arraybuffer/blob), `WebSocket`, missing: `xmlhttprequest2` edge cases (5 pts) |
...
| **Security** (`security`) | 29 | 32 | 3 | High Pass | `window.crypto.subtle`, Subresource Integrity (`integrity`), `postMessage`, `credentials`, `iframe.sandbox`, `iframe.srcdoc`, `security.csp11`, missing: `security.csp10` (legacy CSP 1.0) (3 pts) |
...
| **Storage** (`storage`) | 35 | 35 | 0 | Complete | `window.localStorage`, `window.sessionStorage`, `IndexedDB` (`storage.indexedDB.blob`, `storage.indexedDB.arraybuffer`), Web SQL |
...
| **TOTAL** | **576** | **588** | **12** | **PASSED (>500)** | **Target: > 500 Points (Actual Score: 576 / 588 - 97.96%)** |

---

## Remaining Missing HTML5 Functions to Achieve a Perfect 588 Score (12 Points Total)

To progress from the current **576 baseline** to a **perfect 588 score**, the following remaining missing HTML5 functions and features need implementation:

### 1. Parsing & Doctype Rules (`parsing.doctype`) — 2 Points
- `parsing.doctype` (2 pts): Returning `document.compatMode === 'CSS1Compat'` for `<!DOCTYPE html>` standards mode documents in all parsing context states.

### 2. Form Input Controls (`form.datetime`) — 1 Point
- `form.datetime` (1 pt): Support for legacy/deprecated `<input type="datetime">` element features (`form.datetime.element`, `ui`, `sanitization`, `min`, `max`, `step`, `valueAsNumber`).

### 3. XMLHTTPRequest Level 2 Edge Cases (`communication.xmlhttprequest2`) — 5 Points
- `communication.xmlhttprequest2` (5 pts): Complete mock/network response handling for specific binary array/blob streams and upload progress events during headless XHR execution.

### 4. Legacy Content Security Policy 1.0 (`security.csp10`) — 3 Points
- `security.csp10` (3 pts): Directive evaluation and event reporting for legacy Content Security Policy 1.0 headers.

### 5. Vector Graphics Embedded SVG Images (`svg.image`) — 1 Point
- `svg.image` (1 pt): Native rendering and bitmap decoding support for external `<image>` element targets embedded inside inline SVG markup.
