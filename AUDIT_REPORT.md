...
## 1. Executive Summary
This audit evaluates the current state of the Wisp browser engine, focusing on modern CSS support, incremental layout, the QuickJS-ng based JavaScript subsystem, and rendering backends. Wisp has transitioned to a modernized architecture featuring QuickJS-ng v0.15.1, an incremental layout engine, and advanced CSS support (Grid, Flexbox, Sticky, Subgrid, Container Queries, 3D Transforms, Animations). Wisp achieves an HTML5Test benchmark baseline of **573 / 588 points** (**97.4%** compliance) with zero JS runtime exceptions. Wisp employs a prioritized native-first graphics strategy where platform-native pipelines are compiled and run by default, completely removing the runtime 'auto' backend selection mode to reduce overhead. Blend2D remains a fully optional alternative rendering choice and software fallback backend. Recent milestones include the complete implementation of the Canvas 2D API bridge, dynamic web font loading (CSS Font Loading API `FontFace`/`FontFaceSet`), HLcache reentrancy protection and synchronous retrieval result population, form input sanitization and label resolution, touch/pointer events APIs, out-of-process JavaScript execution with process/origin isolation, standard Fetch/Streams, Shadow DOM v1, HTML5 History APIs, and full integration of the second wave of WebIDL stubs.
...
*   **HTML5Test Score Baseline (573 / 588)**: Standardized test suite execution achieves **573 / 588 points** (**97.4%** compliance) with 100% scores in Scripting (32/32), Resource Loading (7/7), and Offline & Storage (13/13), and 65/66 in Forms & Input, with 0 runtime exceptions or crashes.
*   **QuickJS DOM Bridge Finalizer Context Safety**: Finalizers in generated and manual QuickJS DOM bindings verify context validity (`JS_ContextIsAlive`) prior to context dereferencing, preventing Use-After-Free (UAF) crashes during garbage collection (`JS_RunGC`).
*   **Table Layout Defensive Child & Anonymous Cell Normalization**: Table row containers (`BOX_TABLE_ROW`) automatically wrap non-cell children in anonymous `BOX_TABLE_CELL` boxes per CSS 2.1 §17.2.1, with defensive NULL guards in width and column type calculation passes preventing assertion crashes.
*   **SIMD CSS Delimiter Scanner Fix**: Corrected boundary comparison logic (`a-z`, `A-Z`, `0-9`) in SSE2 and NEON SIMD delimiter scanners (`css_delimiters.h`), ensuring boundary characters like `{`, `}`, `:`, and `;` are matched accurately.
*   **GTK UI Event Loop & Async Script Wait Integration**: Hooked `qjs_execute_pending_all` into GLib event loops via periodic timeouts and pumped GTK UI events during IPC script waits, preserving UI responsiveness and ensuring seamless event bubbling.
*   **Haiku/BeOS Event Loop & Mouse Click Fixes**: Resolved Haiku/BeOS `select()` infinite timeout integer overflow, auto-prefixed scheme-less URL entries, and restored mouse click event dispatch for web elements.
*   **SHM_DOM Cache Expansion & Compact Node Traversal**: Expanded node wrapper cache capacity (`SHM_DOM_MAX_NODES`) from 1024 to 8192 nodes, preserving wrapper identity and expando properties on large DOM trees, and fixed `Element.prototype.closest` node type matching for compact nodes (`node_type == 1`).
...
*   **2514+ WebIDL Stubs Overrides inside stubs_manual_impl.c (3008+ total manual overrides across all impl files)**: Fully implemented, process-hardened, and integrated over 3008+ total manual WebIDL stub overrides across key HTML and DOM interfaces (with 2514+ custom overrides in `stubs_manual_impl.c` alone, fully supporting `HTMLTableElement`, `HTMLFormElement`, `HTMLInputElement`, `HTMLTextAreaElement`, child-traversal properties, document collections, and subsequent waves of stubs), accompanied by rigorous unit/integration test coverage with 100% pass rates.
*   **Storage & Files Polyfills & Microtask Processing**: Full polyfill integration for `Blob`, `File`, `FileReader`, `URL.createObjectURL`/`revokeObjectURL`, and `IndexedDB` family (`IDBFactory`, `IDBOpenDBRequest`, `IDBDatabase`, `IDBTransaction`, `IDBObjectStore`, `IDBRequest`). Microtask execution driven via `qjs_execute_timers`.
*   **Web Crypto API & HTMLIFrameElement Security**: Web Crypto API (`window.crypto.subtle`) backed by OpenSSL/LibreSSL, `HTMLIFrameElement.sandbox` token list binding, and `srcdoc` attribute integration.
*   **Media, Speech Synthesis & Hardware Sensors**: `canPlayType` MIME/codec checks, `Audio`, `TextTrack`, `VTTCue`, `MediaDevices`, `SpeechSynthesis` voices, `navigator.geolocation`, `DeviceOrientation`/`Motion`, `Gamepad`, `BatteryManager`, and Vibration APIs.
*   **Real-time, Communication & Editing APIs**: WebRTC (`RTCPeerConnection`, `RTCDataChannel`), `MessageChannel`/`MessagePort`, `SharedWorker`, `BroadcastChannel`, `WebSocket`, `EventSource`, `contentEditable`, `document.designMode`, `draggable`, and tag-to-constructor HTML5 prototype dispatching.
...
*   **JavaScript URL Scheme Fetcher Pipeline**: Implemented `javascript:` URL scheme fetcher in `src/content/handlers/javascript/fetcher.c` with percent-decoding (`url_unescape`), HTTP 200 header setup (`Content-Type: text/html; charset=utf-8`), and `FETCH_DATA`/`FETCH_FINISHED` callback message dispatches.
*   **Win32 CoreWindow Vertical & Horizontal Scrolling**: Implemented vertical (`nsw32_corewindow_vscroll`) and horizontal (`nsw32_corewindow_hscroll`) scrolling and scrollbar position synchronization for Windows core window instances using `SetScrollInfo`, `GetScrollInfo`, and `ScrollWindowEx` with `SW_INVALIDATE`.
*   **FS Backing Store Memory-Mapped (`mmap`) Retrieval**: Integrated zero-copy `mmap` retrieval (POSIX `mmap` / Windows `MapViewOfFile`) for backing store elements >16KB in `src/content/fs_backing_store.c` alongside clean memory teardown (`entry_destroy_alloc`) and maintenance timer unscheduling on finalization.
*   **Table Border Conflict Resolution Across Row Groups**: Updated CSS 2.1 §17.6.2.1 table border conflict resolution in `src/content/handlers/html/table.c` (`table_used_left_border_for_cell`, `table_used_right_border_for_cell`, `table_used_bottom_border_for_cell`) to traverse row groups (`BOX_TABLE_ROW_GROUP`) for multi-row spanned cells.
*   **Hardened LLCACHE Metadata Deserialization**: Replaced loose `sscanf` parsing in `llcache_process_metadata` with strict `ns_strtoull`/`ns_strtoint` conversion and enforced an upper bound limit (`num_headers <= 10000`) to prevent integer parsing errors or cache metadata poisoning.
*   **NSURL Password Redaction & Qt String Formatting**: Password components in `nsurl_dump` are redacted (`Password: [REDACTED]`) to prevent credential leaks in debug logs, and Qt search provider menu formatting refactored to use safe `QString::arg()` substitution.
*   **Form Select Inline Box Encapsulation & String Length Optimizations**: Encapsulated select control inline text box retrieval (`form_select_get_inline_text_box`) in `form.c`, and cached string lengths (`eterm_len`, `searchstr_len`, `term_len`, `snprintf` return values) outside loops in `searchweb.c`, `save_complete.c`, `urldb`, and `vsnstrjoin`.
