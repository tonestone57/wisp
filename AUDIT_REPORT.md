...
4.  **SIMD-Accelerated UTF-8 processing**: Wrap `libutf8proc` routines with SIMD (AVX2/NEON) vectorization to accelerate both fast-path ASCII/UTF-8 validation and full-path text normalization, case-mapping, and encoding conversions globally, speeding up HTML parsing, text layout, and multi-process IPC transfers. (Complexity: **Medium** | Benefit: **Medium**)
