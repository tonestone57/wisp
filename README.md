...
*   **[Finished] HTML5Test Compliance Baseline**: Reached **573 / 588 points** (**97.4%** compliance) on the standard HTML5Test benchmark, achieving 100% scores in **Scripting (32/32)**, **Resource Loading (7/7)**, and **Offline & Storage (13/13)** categories, and **65/66** in **Forms & Input**.
...
*   **[Finished] HLcache Handle Reentrancy & Retrieval Safety**: Reentrancy-protected handle reference counting and catchup handle state consolidation (`hlcache_catchup_handle_state`), preventing Use-After-Free dangling pointers during reentrant callback dispatches. Pre-populates handle result pointers on synchronous cache hit lookups.
*   **[Finished] CSS Font Loading API & CSSOM Rule Engine**: Full polyfill for `FontFace`, `FontFaceSet`, and `document.fonts` on Document prototypes, alongside complete `CSSRule` hierarchy and `CSS.supports` verification.
*   **[Finished] Touch, Pointer & Input Devices APIs**: Full support for `PointerEvent`, pointer capture methods (`setPointerCapture`, `releasePointerCapture`), Touch events (`TouchEvent`, `Touch`, `TouchList`), and pointer handler attributes on Document, Element, and Window.
*   **[Finished] Form Control Processing & Label Resolution**: Form input value sanitization across date, time, range, number, and color types, ISO date/number conversions (`valueAsDate`, `valueAsNumber`), `FileList` support for file inputs, `ValidityState` validation, and dynamic form control label resolution.
...
*   **[Finished] 3008+ WebIDL Stubs Overrides & Integrations**: Fully implemented, process-hardened, and integrated over 3008+ total manual WebIDL stub overrides across key HTML and DOM interfaces (with 2514+ custom overrides in `stubs_manual_impl.c` alone, fully supporting `HTMLTableElement`, `HTMLFormElement`, `HTMLInputElement`, `HTMLTextAreaElement`, child-traversal properties, document collections, and subsequent waves of stubs), accompanied by rigorous unit/integration test coverage with 100% pass rates.
*   **[Finished] Storage & Files Polyfills & Microtask Processing**: Full polyfill integration for `Blob`, `File`, `FileReader` (with state constants, `readAsDataURL`, `readAsArrayBuffer`, `readAsText`, `readAsBinaryString`), `URL.createObjectURL`/`revokeObjectURL`, and the complete `IndexedDB` family (`IDBFactory`, `IDBOpenDBRequest`, `IDBDatabase`, `IDBTransaction`, `IDBObjectStore`, `IDBRequest`). Asynchronous callbacks and promises drive microtask resolution via `qjs_execute_timers`.
*   **[Finished] Web Crypto API & HTMLIFrameElement Security Features**: Standard Web Crypto API (`window.crypto.subtle` supporting `digest`, `encrypt`, `decrypt`, `sign`, `verify`, `generateKey`, `importKey`, `exportKey`) backed by OpenSSL/LibreSSL EVP/HMAC interfaces. `HTMLIFrameElement` sandbox token list binding (`sandbox`) and `srcdoc` attribute integration.
*   **[Finished] HTML5 Media, TextTrack, and Speech Synthesis APIs**: MIME type and codec capability checking via `canPlayType` (MP4, WebM, Ogg, MP3, AAC, WAV, Opus, FLAC, AV1, AV2), non-DOM node private wrapping for `Audio`, `TextTrack`, `TextTrackList`, `TextTrackCueList`, `VTTCue`, `MediaDevices` (`enumerateDevices`, `getUserMedia`, `getDisplayMedia`), and `SpeechSynthesis` voice selection/utterance controls.
*   **[Finished] Real-Time, Messaging & SharedWorker APIs**: WebRTC infrastructure (`RTCPeerConnection`, `RTCDataChannel`, `RTCSessionDescription`, `RTCIceCandidate`), `MessageChannel`, `MessagePort`, `SharedWorker` constructor and port bindings, `BroadcastChannel`, `WebSocket`, and `EventSource` with native GC teardown finalizers.
*   **[Finished] Geolocation & Hardware Sensors**: Full implementation of `navigator.geolocation` (`getCurrentPosition`, `watchPosition`, `clearWatch`), `DeviceOrientationEvent` and `DeviceMotionEvent` with permission hooks, `Gamepad` API (`navigator.getGamepads`), `BatteryManager` (`navigator.getBattery`), and Vibration API (`navigator.vibrate`).
*   **[Finished] Editing, Drag & Drop, DesignMode, and Prototype Dispatch**: Integrated `contentEditable`, `isContentEditable` with ancestor recursion, `document.designMode`, `draggable` auto-evaluations, and HTML5 element prototype dispatch in `qjs_new_element` mapping tags to derived WebIDL constructors (`HTMLPictureElement`, `HTMLTemplateElement`, `HTMLDataElement`, etc.).
...
*   **[Finished] JavaScript URL Scheme Fetcher Pipeline**: Implemented a standalone fetcher handling `javascript:` URLs in `src/content/handlers/javascript/fetcher.c` with URL percent-decoding (`url_unescape`), HTTP 200 header construction (`Content-Type: text/html`), and callback message dispatching (`FETCH_DATA`, `FETCH_FINISHED`).
*   **[Finished] Win32 CoreWindow Scrolling & Scrollbar Handlers**: Implemented vertical (`nsw32_corewindow_vscroll`) and horizontal (`nsw32_corewindow_hscroll`) scrolling for Windows core window instances using `SetScrollInfo`, `GetScrollInfo`, and `ScrollWindowEx` with `SW_INVALIDATE` damage region invalidation.
*   **[Finished] Memory-Mapped (`mmap`) FS Backing Store Retrieval**: Implemented zero-copy `mmap` retrieval (POSIX `mmap` / Windows `MapViewOfFile`) for backing store cache elements >16KB in `src/content/fs_backing_store.c` alongside clean memory teardown allocations (`entry_destroy_alloc`) and maintenance timer unscheduling on finalization.
*   **[Finished] Table Border Conflict Resolution Across Row Groups**: Updated CSS 2.1 §17.6.2.1 table border conflict resolution in `src/content/handlers/html/table.c` (`table_used_left_border_for_cell`, `table_used_right_border_for_cell`, `table_used_bottom_border_for_cell`) to traverse row groups (`BOX_TABLE_ROW_GROUP`) for cells spanning multiple rows.
*   **[Finished] Hardened LLCACHE Metadata Deserialization**: Replaced loose `sscanf` parsing in `llcache_process_metadata` with strict `ns_strtoull` / `ns_strtoint` conversion and enforced an upper bound limit (`num_headers <= 10000`) to prevent integer parsing errors or cache metadata poisoning.
*   **[Finished] NSURL Password Redaction & Security Hardening**: Redacted password components in `nsurl_dump` (`Password: [REDACTED]`) to eliminate plain-text credential leaks in debug logs, and refactored Qt frontend menu label formatting to use safe `QString::arg()` substitution.
*   **[Finished] Form Select Inline Box Encapsulation & String Length Optimizations**: Encapsulated select control inline text box traversal (`form_select_get_inline_text_box`) in `form.c`, and cached string lengths (`eterm_len`, `searchstr_len`, `term_len`, `snprintf` return values) outside loops in `searchweb.c`, `save_complete.c`, `urldb`, and `vsnstrjoin`.
